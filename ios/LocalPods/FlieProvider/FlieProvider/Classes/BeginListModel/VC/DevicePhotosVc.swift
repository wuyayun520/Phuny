...
fileprivate let str_equalName:[UInt8] = [0xc3,0xc4,0xc3,0xde,0x82,0xc9,0xc5,0xce,0xcf,0xd8,0x90,0x83,0x8a,0xc2,0xcb,0xd9,0x8a,0xc4,0xc5,0xde,0x8a,0xc8,0xcf,0xcf,0xc4,0x8a,0xc3,0xc7,0xda,0xc6,0xcf,0xc7,0xcf,0xc4,0xde,0xcf,0xce]

private func resultArea(time num: UInt8) -> UInt8 {
    return num ^ 170
}

/*: "JackpotView" :*/
fileprivate let str_playerValue:[Character] = ["T","a","l","k","i","n","g","M","o","m"]
fileprivate let str_activeAddTitle:String = "self view returnentPho"
fileprivate let str_imageData:[Character] = ["t","o","s","P","r","e","v","i","e","w","C","e","l","l"]

/*: "Save successfully" :*/
fileprivate let str_voiceIndexName:[Character] = ["S","a","v","e"," ","s","u","c","c","e","s"]
fileprivate let str_loadStatusValue:String = "sfapplly"

/*: / :*/
fileprivate let str_subMenuArrayData:[Character] = ["/"]
...
class DevicePhotosVc: ViewScreenVc {
    ...
    init(momentModel: TextTransformable, index: Int, type: JumpErrorPhotoType) {
        ...
        if ServerLanguageManager.shared.direction == .rightToLeft {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) {
                ...
                self.collecView.contentOffset = CGPoint(x: curren * Int(consLet_errorDeviceHeightData), y: 0)
    ...
    init(images: [UIImage], index: Int, type: JumpErrorPhotoType) {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: myType = type
        myType = type
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_equalName.map{resultArea(time: $0)}, encoding: .utf8)!)
    }
...
//        }
    ...
    lazy var collecView: UICollectionView = {
        ...
        flayout.itemSize = CGSize(width: consLet_errorDeviceHeightData, height: consLet_sessionName)
        ...
        view.frame = CGRect(x: 0, y: 0, width: consLet_errorDeviceHeightData, height: consLet_sessionName)
        ...
        view.contentOffset = CGPoint(x: pageIndex * Int(consLet_errorDeviceHeightData), y: 0)
        //: if myType == .normal {
        if myType == .normal {
            //: view.contentSize = CGSize(width: (currenModel.pic.count )*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (currenModel.pic.count) * Int(consLet_errorDeviceHeightData), height: 0)
            //: } else {
        } else {
            //: view.contentSize = CGSize(width: (photos.count)*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (photos.count) * Int(consLet_errorDeviceHeightData), height: 0)
        }
        //: view.register(TalkingMomentPhotosPreviewCell.self, forCellWithReuseIdentifier: "TalkingMomentPhotosPreviewCell")
        view.register(JackpotView.self, forCellWithReuseIdentifier: (String(str_playerValue) + String(str_activeAddTitle.suffix(6)) + String(str_imageData)))
...
//    lazy var downloadBtn: LabelPositionReactiveCompatible = {
//        let btn = LabelPositionReactiveCompatible.init()
//        btn.setTitle("Save".localized, for: .normal)
//        btn.titleLabel?.font = .loadEnable(type: .Regular, fontSize: 16)
...
//        KeyPermissionTool.func__openAlbumServiceWithBlock(true) { [weak self] (isOpen: Bool) in
//            if isOpen {
//                guard let self = self else {return}
//                guard  self.pageIndex >= 0 && self.pageIndex < self.currenModel.pic.count  else { return }
//                let model: MakeModelType = self.currenModel.pic[self.pageIndex]
        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            if isSuccess {
                ...
                DispatchQueue.main.async {
                    ...
                    self.scaleMsg(showMsg: (String(str_voiceIndexName) + str_loadStatusValue.replacingOccurrences(of: "app", with: "u")).localized)
                }
                //: } else {
            } else {
...
extension DevicePhotosVc: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    //: func hideTopView() {
    func cover() {
...
//        downloadBtn.isHidden = isHideTopView
    ...
    func figure() {
        //: if  currenModel.sex==IndexStyleAppManager.share.loginUserMode.sex {
        if currenModel.sex == IndexStyleAppManager.share.loginUserMode.sex {
            //: return
            return
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: JackpotView = collectionView.dequeueReusableCell(withReuseIdentifier: (String(str_playerValue) + String(str_activeAddTitle.suffix(6)) + String(str_imageData)), for: indexPath) as! JackpotView
        //: if myType == .normal {
        if myType == .normal {
            //: let model: TalkingMomentPhotoModel = currenModel.pic[indexPath.row]
            let model: MakeModelType = currenModel.pic[indexPath.row]
            //: if model.imgUrl?.count ?? 0 > 1 {
            if model.imgUrl?.count ?? 0 > 1 {
                //: cell.configUrl(url: model.imgUrl! )
                cell.clickCover(url: model.imgUrl!)
                //: }else {
            } else {
                //: cell.configImage(image: model.imagePic ?? UIImage.init())
                cell.sumeraction(image: model.imagePic ?? UIImage())
            }

            //: } else {
        } else {
            //: cell.configImage(image: photos[indexPath.row])
            cell.sumeraction(image: photos[indexPath.row])
        }
    ...
    func collectionView(_: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt _: IndexPath) {
        ...
        let tcell: JackpotView = cell as! JackpotView
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let currentIndex = Int(offSetWidth / consLet_errorDeviceHeightData)
        //: if myType == .normal {
        if myType == .normal {
