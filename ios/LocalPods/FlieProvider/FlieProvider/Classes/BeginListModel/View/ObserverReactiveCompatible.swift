...
fileprivate let str_placeValue:[UInt8] = [0x9f,0xa4,0x9f,0xaa,0x5e,0x99,0xa5,0x9a,0x9b,0xa8,0x70,0x5f,0x56,0x9e,0x97,0xa9,0x56,0xa4,0xa5,0xaa,0x56,0x98,0x9b,0x9b,0xa4,0x56,0x9f,0xa3,0xa6,0xa2,0x9b,0xa3,0x9b,0xa4,0xaa,0x9b,0x9a]

fileprivate func blotLab(view num: UInt8) -> UInt8 {
...
class ObserverReactiveCompatible: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_placeValue.map{blotLab(view: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var progressView: KeyProgressView = {
        ...
        let v = KeyProgressView(frame: self.frame, progressBgColor: self.progressBgColor, progressPlayedColor: self.progressPlayedColor, progressCachedColor: self.progressCachedColor)
    ...
    private lazy var panGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(targetTing(gesture:)))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(headGesture(gesture:)))
        //: return v
        return v
        //: }()
    }()
...
extension ObserverReactiveCompatible {
    ...
    @objc func targetTing(gesture: UIPanGestureRecognizer) {
        // 移动的距离
    ...
    @objc func headGesture(gesture: UIPanGestureRecognizer) {
