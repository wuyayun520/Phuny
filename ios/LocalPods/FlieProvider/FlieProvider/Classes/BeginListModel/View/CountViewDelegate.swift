...
fileprivate let str_tableTopData:[UInt8] = [0x88,0x8f,0x88,0x95,0xc9,0x82,0x8e,0x85,0x84,0x93,0xdb,0xc8,0xc1,0x89,0x80,0x92,0xc1,0x8f,0x8e,0x95,0xc1,0x83,0x84,0x84,0x8f,0xc1,0x88,0x8c,0x91,0x8d,0x84,0x8c,0x84,0x8f,0x95,0x84,0x85]

private func panPoint(from num: UInt8) -> UInt8 {
    return num ^ 225
}

/*: "LogReusableView" :*/
fileprivate let str_acceptData:String = "head list messageTalkin"
fileprivate let str_timeName:[Character] = ["g","M","o","m","e","n","t","P","h","o","t","o","s","C","e","l","l"]
...
class CountViewDelegate: UICollectionView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_tableTopData.map{panPoint(from: $0)}, encoding: .utf8)!)
    }

    //: func configModel(model: TalkingMomentModel) {
    func rowApp(model: TextTransformable) {
        //: cunrrenModel = model
        cunrrenModel = model
        //: self.reloadData()
        self.reloadData()
...
extension CountViewDelegate: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, insetForSectionAt _: Int) -> UIEdgeInsets {
        ...
        if cunrrenModel.pic.count == 4 {
            ...
            return UIEdgeInsets(top: 0, left: 0, bottom: 0, right: consLet_errorDeviceHeightData - 200 - 30)
    ...
    func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, sizeForItemAt _: IndexPath) -> CGSize {
        ...
        if cunrrenModel.pic.count == 1 {
            ...
            var width = isMyhost ? (consLet_errorDeviceHeightData - 30 - 24 - 30) / 3 : (consLet_errorDeviceHeightData - 30 - 24) / 3
    ...
    func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, referenceSizeForHeaderInSection _: Int) -> CGSize {
        ...
        return CGSize(width: consLet_errorDeviceHeightData, height: 1)
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: LogReusableView = collectionView.dequeueReusableCell(withReuseIdentifier: (String(str_acceptData.suffix(6)) + String(str_timeName)), for: indexPath) as! LogReusableView
        //: let model: TalkingMomentPhotoModel = cunrrenModel.pic[indexPath.row]
        let model: MakeModelType = cunrrenModel.pic[indexPath.row]
        //: cell.configUrl(url: model.thumbUrl ?? "")
        cell.extensionPhone(url: model.thumbUrl ?? "")
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let vc = DevicePhotosVc(momentModel: cunrrenModel, index: indexPath.row, type: .normal)
...
extension CountViewDelegate {
    ...
    private func modelSubviews() {
        ...
        register(LogReusableView.self, forCellWithReuseIdentifier: (String(str_acceptData.suffix(6)) + String(str_timeName)))
        //: dataSource = self
        dataSource = self
        //: delegate = self
        delegate = self
