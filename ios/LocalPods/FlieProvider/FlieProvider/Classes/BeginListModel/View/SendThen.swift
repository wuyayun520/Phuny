...
fileprivate let str_effectName:[UInt8] = [0xa9,0xae,0xa9,0xb4,0xe8,0xa3,0xaf,0xa4,0xa5,0xb2,0xfa,0xe9,0xe0,0xa8,0xa1,0xb3,0xe0,0xae,0xaf,0xb4,0xe0,0xa2,0xa5,0xa5,0xae,0xe0,0xa9,0xad,0xb0,0xac,0xa5,0xad,0xa5,0xae,0xb4,0xa5,0xa4]

private func displayEqual(tool num: UInt8) -> UInt8 {
    return num ^ 192
}

/*: "F5F5F5" :*/
fileprivate let str_viewName:[Character] = ["F","5","F","5","F","5"]
...
class SendThen: UITableViewCell {
    ...
    var videoView = MechanicallyView()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_effectName.map{displayEqual(tool: $0)}, encoding: .utf8)!)
    }
    ...
    func toSex(isListTableCell: Bool) {
    ...
    lazy var userInfoView: SomebodyView = {
        //: let  view = TalkingMomentUserInfoView.init()
        let view = SomebodyView()
    ...
    lazy var textContentView: QuantityView = {
    ...
    lazy var photoView: CountViewDelegate = {
        ...
        let view = CountViewDelegate(frame: CGRect(x: 0, y: 0, width: 0, height: 0), collectionViewLayout: flayout)
        //: backView.addSubview(view)
        backView.addSubview(view)
        //: return view
        return view
    ...
    lazy var bottomView: MomentView = {
...
extension SendThen {
    ...
    func area(model: TextTransformable) {
        //: cunrrenModel = model
        cunrrenModel = model
        //: userInfoView.isHidden = false
        userInfoView.isHidden = false
        //: userInfoView.configInfo(model: model)
        userInfoView.exceptImageSelected(model: model)
        //: photoView.isMyhost = self.isMyhost
        photoView.isMyhost = self.isMyhost
        //: textContentView.configText(text: model.content ?? "", isMomentDetail: model.isMomentDetail ?? false)
        textContentView.ingredient(text: model.content ?? "", isMomentDetail: model.isMomentDetail ?? false)

        //: if model.momentType == MomentType.Photo.rawValue || model.momentType == MomentType.Live.rawValue {
        if model.momentType == AddDoinglyClusterLiteral.Photo.rawValue || model.momentType == AddDoinglyClusterLiteral.Live.rawValue {
...
//            }
            ...
            photoView.rowApp(model: cunrrenModel)
...
//            }
            ...
            videoView.sumryTop(model: model.videoInfo!, isTop: model.pinStatus)
        }

        //: bottomView.configModel(model: cunrrenModel)
        bottomView.eventModel(model: cunrrenModel)
...
//        }
    ...
    @objc func cardView() {
        ...
        let vc = JewishCalendarMonthViewController(videoPath: cunrrenModel.videoInfo!.videoUrl)
        //: if cunrrenModel.sex != IndexStyleAppManager.share.loginUserMode.sex {
        if cunrrenModel.sex != IndexStyleAppManager.share.loginUserMode.sex {
    ...
    func backstopSend() {
        ...
        if cunrrenModel.momentType == AddDoinglyClusterLiteral.Video.rawValue {
...
extension SendThen {
    ...
    private func equalSendHandle() {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(cardView))
