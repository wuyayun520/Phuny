...
fileprivate let str_pushValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
class JackpotView: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_pushValue.reversed(), encoding: .utf8)!)
    }

    //: func customUI() {
    func toCell() {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(imageViewClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(capacity))
        ...
        let doubletap = UITapGestureRecognizer(target: self, action: #selector(nominateTap))
...
extension JackpotView: UIScrollViewDelegate {
    //: @objc func imageViewClick(tap: UIGestureRecognizer) {
    @objc func capacity(tap _: UIGestureRecognizer) {
        //: if sblock != nil {
        if sblock != nil {
            //: sblock()
            sblock()
    ...
    @objc func nominateTap(tap: UIGestureRecognizer) {
    ...
    func clickCover(url: String) {
        //: imageView.setUrlImage(urlStr: url)
        imageView.underPause(urlStr: url)
    }

    //: func configImage(image: UIImage) {
    func sumeraction(image: UIImage) {
        //: imageView.image = image
        imageView.image = image
    }

    //: func refreshImageContainerViewCenter () {
    func financialSupport() {
