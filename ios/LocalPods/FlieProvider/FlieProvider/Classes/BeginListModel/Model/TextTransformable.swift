...
/*: "id" :*/
fileprivate let str_tapTitle:String = "iplayer"
...
struct TextTransformable: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
...
struct MakeModelType: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
enum AddDoinglyClusterLiteral: String {
    //: case nones  = "0"
    case nones = "0"
    //: case Photo = "1"
    case Photo = "1"
...
struct FailHandyJSON: HandyJSON {
    //: var videoUrl = ""
    var videoUrl = ""
    //: var imgUrl = ""
    var imgUrl = ""
    ...
    var status: BackgroundCompatibleValue?
}

///
//: enum MomentVideoStatus: String {
enum BackgroundCompatibleValue: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension TextTransformable {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func noneEqual(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            //: textHeight = Int(TalkingMomentTextSizeView.caculateSizeWithText(text: content!, size: CGSize.init(width: ScreenWidth-54, height: CGFloat(MAXFLOAT)), font: UIFont.loadEnable(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber==0 ? 20:0))
            textHeight = Int(SequenceReactiveCompatible.even(text: content!, size: CGSize(width: consLet_errorDeviceHeightData - 54, height: CGFloat(MAXFLOAT)), font: UIFont.loadEnable(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == AddDoinglyClusterLiteral.Photo.rawValue || momentType == AddDoinglyClusterLiteral.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
