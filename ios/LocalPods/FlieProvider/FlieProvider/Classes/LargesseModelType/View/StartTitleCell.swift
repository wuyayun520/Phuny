...
fileprivate let str_upText:[UInt8] = [0x7,0x0,0x7,0x1a,0x46,0xd,0x1,0xa,0xb,0x1c,0x54,0x47,0x4e,0x6,0xf,0x1d,0x4e,0x0,0x1,0x1a,0x4e,0xc,0xb,0xb,0x0,0x4e,0x7,0x3,0x1e,0x2,0xb,0x3,0xb,0x0,0x1a,0xb,0xa]

private func withTap(view num: UInt8) -> UInt8 {
    return num ^ 110
}

/*: "Pending review" :*/
fileprivate let str_methodData:[Character] = ["P","e","n","d","i","n","g"," "]
fileprivate let str_appearName:String = "rebottomew"
...
class StartTitleCell: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_upText.map{withTap(view: $0)}, encoding: .utf8)!)
    }
...
extension StartTitleCell {
    //: func setGreetTextCell(model: TalkingGreetModel, row: Int, isLast: Bool) {
    func withSize(model: JumpHandyJSON, row: Int, isLast: Bool) {
        //: index = row
        index = row
        //: islast = isLast
        islast = isLast
        ...
        if model.status == 0 {
            ...
            statusLb.text = (String(str_methodData) + str_appearName.replacingOccurrences(of: "bottom", with: "vi")).localized
