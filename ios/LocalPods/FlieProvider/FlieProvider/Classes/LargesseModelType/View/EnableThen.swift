...
/*: "#AAAAAA" :*/
fileprivate let str_countErrBroadcastName:String = "#AAAAAAplay style info true path"

/*: " No more than 10 characters" :*/
fileprivate let str_hiddenName:[Character] = [" ","N","o"," ","m","o","r","e"," ","t","h","a","n"," ","1","0"," ","c","h"]
fileprivate let str_groupValue:String = "framea"
fileprivate let str_topValue:[Character] = ["c","t","e","r","s"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let str_errorValue:[UInt8] = [0x4f,0x54,0x4f,0x5a,0xe,0x49,0x55,0x4a,0x4b,0x58,0x20,0xf,0x6,0x4e,0x47,0x59,0x6,0x54,0x55,0x5a,0x6,0x48,0x4b,0x4b,0x54,0x6,0x4f,0x53,0x56,0x52,0x4b,0x53,0x4b,0x54,0x5a,0x4b,0x4a]

fileprivate func makeAt(model num: UInt8) -> UInt8 {
...
class EnableThen: UIView {
    ...
    private var alertConfig: ResourceAlertConfig
    ...
    private lazy var inputTView: EconomicAidReactiveCompatible = {
        ...
        input.placeholder = (String(str_hiddenName) + str_groupValue.replacingOccurrences(of: "frame", with: "ar") + String(str_topValue)).localized
        ...
        input.backgroundColor = .primaryColorBarContent()
    ...
    private lazy var leftBtn: UIButton = {
        ...
        leftBtn.layer.borderColor = UIColor.labelSumervalMedium().cgColor
        //: leftBtn.layer.borderWidth = 1
        leftBtn.layer.borderWidth = 1
        //: leftBtn.addTarget(self, action: #selector(leftClick), for: .touchUpInside)
        leftBtn.addTarget(self, action: #selector(source), for: .touchUpInside)
        //: return leftBtn
        return leftBtn
        //: }()
    }()
    ...
    private lazy var rightBtn: UIButton = {
        ...
        rightBtn.setBackgroundImage(UIImage.onCornerTo(colors: UIColor.gradient(), size: CGSize(width: 267, height: 50)), for: .normal)
        ...
        rightBtn.setBackgroundImage(UIImage.onCornerTo(colors: UIColor.appInVisualProperty(), size: CGSize(width: 267, height: 50)), for: .disabled)
        ...
        rightBtn.addTarget(self, action: #selector(transplant), for: .touchUpInside)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_errorValue.map{makeAt(model: $0)}, encoding: .utf8)!)
    }

    //: func changeRightBtn(title: String, isTouch: Bool) {
    func piece(title: String, isTouch: Bool) {
        ...
        //: if !isTouch {
        if !isTouch {
            //: rightBtn.setBackgroundImage(UIImage.nearTap(name: alertConfig.rightBackgroundImagefalse), for: .normal)
            rightBtn.setBackgroundImage(UIImage.nearTap(name: alertConfig.rightBackgroundImagefalse), for: .normal)
            //: } else {
        } else {
            //: let image = UIImage.nearTap(name: alertConfig.rightBackgroundImage).withTintColor(.appThemeColor())
            let image = UIImage.nearTap(name: alertConfig.rightBackgroundImage).withTintColor(.labelSumervalMedium())
...
extension EnableThen: UITextViewDelegate {
    //: func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
    func searchEqual(_ textField: UITextField, shouldChangeCharactersIn _: NSRange, replacementString string: String) -> Bool {
    ...
    func turnBack(_ textField: UITextField) -> Bool {
    ...
    func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > alertConfig.lengthLimit {
            ...
            if textNum > alertConfig.lengthLimit && alertConfig.lengthLimit > 0 {
                ...
                textView.text = checkedLine(index: alertConfig.lengthLimit, text: textContent)
            }
        }
    }

    //: private func string_prefix(index: Int, text: String) -> String {
    private func checkedLine(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
