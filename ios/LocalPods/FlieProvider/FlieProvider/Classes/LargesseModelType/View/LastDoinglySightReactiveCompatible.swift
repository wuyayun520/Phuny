...
fileprivate let str_arrayValue:[UInt8] = [0x44,0x43,0x44,0x59,0x5,0x4e,0x42,0x49,0x48,0x5f,0x17,0x4,0xd,0x45,0x4c,0x5e,0xd,0x43,0x42,0x59,0xd,0x4f,0x48,0x48,0x43,0xd,0x44,0x40,0x5d,0x41,0x48,0x40,0x48,0x43,0x59,0x48,0x49]

private func viewRowTo(hidden num: UInt8) -> UInt8 {
    return num ^ 45
}

/*: "StepTagCell" :*/
fileprivate let str_pathCrushName:String = "user view target atTalk"
fileprivate let str_indicatorVisualDataName:[Character] = ["l"]
...
// __CLOSE_PRINT__
//
//  LastDoinglySightReactiveCompatible.swift
//  AbroadTalking
//
...
typealias DeleteTagBlock = (ErrorEquatable) -> Void

//: class TalkingTagView: UIView {
class LastDoinglySightReactiveCompatible: UIView {
    //: var deleteBlock: DeleteTagBlock!
    var deleteBlock: DeleteTagBlock!
    //: var titles = [UserSeleteTagModel]()
    var titles = [ErrorEquatable]()
    //: var tagContentAlignment = TagContentAlignment.left {
    var tagContentAlignment = ServerTextAlignment.left {
        //: didSet {
        didSet {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_arrayValue.map{viewRowTo(hidden: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var layout: IndexMakeReactiveCompatible = {
        ...
        let lay = IndexMakeReactiveCompatible()
...
extension LastDoinglySightReactiveCompatible: UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: (String(str_pathCrushName.suffix(4)) + "ingTagCel" + String(str_indicatorVisualDataName)), for: indexPath) as! StepTagCell
...
extension LastDoinglySightReactiveCompatible: ItemThen {
    ...
    func group(_: IndexMakeReactiveCompatible, collectionView _: UICollectionView, sizeForSupplementaryElementOfKind _: String, at _: Int) -> CGSize {
...
extension LastDoinglySightReactiveCompatible {
    //: func setframe(frame: CGRect) {
    func noneNext(frame _: CGRect) {
        //: collectionView.frame = self.bounds
        collectionView.frame = self.bounds
        //: collectionView.backgroundColor = .white
        collectionView.backgroundColor = .white
        //: collectionView.register(TalkingTagCell.self, forCellWithReuseIdentifier: "TalkingTagCell")
        collectionView.register(StepTagCell.self, forCellWithReuseIdentifier: (String(str_pathCrushName.suffix(4)) + "ingTagCel" + String(str_indicatorVisualDataName)))
