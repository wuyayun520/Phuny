...
class ServerTalkingThen: NSObject {
    //: var infoModel: TalkingVideoChatModel?          
    var infoModel: PastChatModel?          // 视频通话model
    //: var config = TalkingVideoChatConfig()          
    var config = TitleChatConfig()          // 视频渲染配置类
    ...
    private var trtcView: ModelKeyView?    // 预览view（使用商汤美颜）
...
extension ServerTalkingThen {
    ...
    func colorCamera(_ frontCamera: Bool) {
        ...
        self.haulUp(front: frontCamera)
    }
    
    /// 摄像头开启与关闭
    /// - Parameter openCamera: 是否开启
    //: func useCamera(_ openCamera: Bool) {
    func statusCamera(_ openCamera: Bool) {
    ...
    func activity() {
        ...
        DispatchQueue.main.async {
            ...
            self.actionNow(smallWindow: self.infoModel?.renderPostion != .selfOnBg)
...
extension ServerTalkingThen {
    ...
    private func haulUp(front: Bool) {
        //: guard SenseTime_Use == true else {
        guard consLet_matchData == true else {
        ...
        self.trtcView?.makeShow(isFront: front)
    }
    
    /// 是否开启摄像头
    /// - Parameter isOpen: 是否开启
    //: private func setTRTCCamera(isOpen: Bool) {
    private func set(isOpen: Bool) {
        //: guard SenseTime_Use == true else {
        guard consLet_matchData == true else {
            //: if isOpen {
            if isOpen {
        ...
        self.trtcView?.setUpBarb(isOpen: isOpen)
...
extension ServerTalkingThen {
    ...
    func beautyFile() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(saveBackgroundAppTakeTheField(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(appChange(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(userTableNotifAdd(notif:)),
    ...
    @objc private func saveBackgroundAppTakeTheField(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
            // 静音
    ...
    @objc private func appChange(notif: Notification?) {
        //: if notif == nil { 
    ...
    @objc private func userTableNotifAdd(notif: Notification) {
    ...
    @objc func doingline(notif: Notification) {
...
extension ServerTalkingThen {
    ...
    func actionNow(smallWindow: Bool) {
        //: guard SenseTime_Use == true else {
        guard consLet_matchData == true else {
        ...
        if self.trtcView == nil {
            ...
            self.trtcView = ModelKeyView()
        }
        //: self.trtcView?.startPreView(isSmallWindow: smallWindow)
        self.trtcView?.dismissDay(isSmallWindow: smallWindow)
        //: if smallWindow { 
...
//    }
    ...
    private func imageView() {
        ...
        //: if SenseTime_Use {
        if consLet_matchData {
            //: self.trtcView?.stopPreView()
            self.trtcView?.spectralColor()
        //: } else {
        } else {
            //: self.isOpenTRTCPreview = false
            self.isOpenTRTCPreview = false
