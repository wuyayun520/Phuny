
//: Declare String Begin

/*: "0,0" :*/
fileprivate let str_requestName:String = "development,development"

/*: "dist/loungePlus" :*/
fileprivate let str_dataName:[Character] = ["d","i","s","t","/","l","o","u","n","g","e"]
fileprivate let str_nearAdjustValue:[Character] = ["P","l","u","s"]

/*: "mf/freeCall/getVideoInfo" :*/
fileprivate let str_sessionGiftName:[Character] = ["m","f","/","f","r","e","e","C","a","l","l"]
fileprivate let str_equalValue:[Character] = ["/","g","e","t","V","i","d"]
fileprivate let str_thumbBackValue:[Character] = ["e","o","I","n","f","o"]

/*: "fromType" :*/
fileprivate let str_sendValue:[UInt8] = [0x65,0x70,0x79,0x54,0x6d,0x6f,0x72,0x66]

/*: "videoId" :*/
fileprivate let str_dailyHiddenData:[Character] = ["v","i","d","e","o","I","d"]

/*: "uid" :*/
fileprivate let str_remoteImageValue:String = "uframe"

/*: "type" :*/
fileprivate let str_cellName:[Character] = ["t","y","p","e"]

/*: "logId" :*/
fileprivate let str_colorData:[Character] = ["l","o","g","I","d"]

/*: "duration" :*/
fileprivate let str_showText:String = "duratvalue"
fileprivate let str_callData:[Character] = ["o","n"]

/*: "stopUid" :*/
fileprivate let str_effectText:[Character] = ["s","t","o","p","U","i"]
fileprivate let str_currentName:[Character] = ["d"]

/*: "mf/freeCall/operation" :*/
fileprivate let str_maxName:[Character] = ["m","f","/","f","r","e","e","C","a","l","l","/","o","p","e","r","a","t","i","o","n"]

//: Declare String End

// __DEBUG__
// __CLOSE_PRINT__
//
//  NameCallManager.swift
//  AbroadTalking
//
//  Created by Charlotte on 2023/11/6.
//

//: import UIKit
import UIKit

/// 主动拨打视频弹窗管理
//: class TalkingVideoInitivCallManager: NSObject {
class NameCallManager: NSObject {
    //: private var videoCallTimer: Timer?
    private var videoCallTimer: Timer? /// timer
    //: var videoCallModel = TalkingVideoInitivCallModel.init()
    var videoCallModel = AstatineMeasurable()
    //: private var countDown = 0
    private var countDown = 0 /// x秒后显示主动拨打弹窗
    //: private var intervalDown = 0
    private var intervalDown = 0 /// 间隔时间x秒
    //: private var timer_Duration: TimeInterval = 0
    private var timer_Duration: TimeInterval = 0 /// 时间间隔 初次3s，后续90s

    //: static let shared = TalkingVideoInitivCallManager()
    static let shared = NameCallManager()

    //: private override init() {
    override private init() {
        //: super.init()
        super.init()
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(stop_VideoCallTimer),
                                               selector: #selector(numbererval),
                                               //: name: DID_LOGIN_OUT_SUCCESS_NOTIFICATION,
                                               name: consLet_askData,
                                               //: object: nil)
                                               object: nil)
    }

    //: override func copy() -> Any { return self }
    override func copy() -> Any { return self }
    //: override func mutableCopy() -> Any { return self }
    override func mutableCopy() -> Any { return self }
}

// MARK: - Event

//: extension TalkingVideoInitivCallManager {
extension NameCallManager {
    //: private func distroryNotif() {
    private func userShow() {
        //: NotificationCenter.default.removeObserver(self)
        NotificationCenter.default.removeObserver(self)
    }

    //: func setObserver() {
    func keyMakeObserver() {
        //: guard IndexStyleAppManager.share.loginUserMode.callPopupSetting != "0,0" else {
        guard IndexStyleAppManager.share.loginUserMode.callPopupSetting != "0,0" else {
            //: distroryNotif()
            userShow()
            //: return
            return
        }
        //: let arr = IndexStyleAppManager.share.loginUserMode.callPopupSetting .split(separator: ",")
        let arr = IndexStyleAppManager.share.loginUserMode.callPopupSetting.split(separator: ",")
        //: if arr.count > 1 {
        if arr.count > 1 {
            //: countDown = Int(arr[0]) ?? 0
            countDown = Int(arr[0]) ?? 0
            //: intervalDown = Int(arr[1]) ?? 0
            intervalDown = Int(arr[1]) ?? 0
        }
        //: DispatchQueue.main.asyncAfter(deadline: DispatchTime.now()+Double(countDown)) {
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + Double(countDown)) {
            //: self.req_getVideoInfo()
            self.addDown()
        }
        /// 充值成功
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(paySuccessTool),
                                               selector: #selector(upTotal),
                                               //: name: USER_UPDATE_RESIDUALINFO_NOTIFICATION,
                                               name: consLet_timeKeyValue,
                                               //: object: nil)
                                               object: nil)
    }

    /// 开启x秒倒计时
    //: func initVideoCallTimer() {
    func mix() {
        //: self.timer_Duration = TimeInterval(intervalDown)
        self.timer_Duration = TimeInterval(intervalDown)
        //: if videoCallTimer == nil {
        if videoCallTimer == nil {
            //: videoCallTimer = Timer.scheduledTimer(timeInterval: timer_Duration, target: self, selector: #selector(req_getVideoInfo), userInfo: nil, repeats: true)
            videoCallTimer = Timer.scheduledTimer(timeInterval: timer_Duration, target: self, selector: #selector(addDown), userInfo: nil, repeats: true)
            //: RunLoop.current.add(videoCallTimer!, forMode: RunLoop.Mode.common)
            RunLoop.current.add(videoCallTimer!, forMode: RunLoop.Mode.common)
        }
    }

    //: private func videoCallTimerPopView() {
    private func selected() {
        /// 当前显示半屏充值页或订阅页时，不显示弹窗
        //: if currentViewController()?.isKind(of: TalkingWebViewController.self) == true {
        if userRead()?.isKind(of: EqualViewController.self) == true {
            //: let vc = currentViewController() as! TalkingWebViewController
            let vc = userRead() as! EqualViewController
            //: if vc.urlStr.contains(WebUrlType.RechargeToVideoInitivHalfPage.rawValue) || vc.urlStr.contains("dist/loungePlus") {
            if vc.urlStr.contains(AutomaticallyCollection.RechargeToVideoInitivHalfPage.rawValue) || vc.urlStr.contains((String(str_dataName) + String(str_nearAdjustValue))) {
                //: return
                return
            }
        }
        /// 当前在随机匹配页，视频通话页，不显示弹窗
        //: if currentViewController()?.isKind(of: TalkingRandomVideoViewController.self) == true || currentViewController()?.isKind(of: TalkingVideoInitivCallVC.self) == true {
        if userRead()?.isKind(of: FailRecognizerDelegate.self) == true || userRead()?.isKind(of: ContextCallVc.self) == true {
            //: return
            return
        }
        //: TalkingPopupWindowManager.shared.videoCallPopUpWindow()
        QuantityWindowManager.shared.captureContainer()
        //: stop_VideoCallTimer()
        numbererval()
    }

    /// 停止
    //: @objc func stop_VideoCallTimer() {
    @objc func numbererval() {
        //: if self.videoCallTimer != nil {
        if self.videoCallTimer != nil {
            //: self.videoCallTimer?.invalidate()
            self.videoCallTimer?.invalidate()
            //: self.videoCallTimer = nil
            self.videoCallTimer = nil
        }
    }

    ///  充值成功后，跳转到callTab页面
    //: @objc private func paySuccessTool() {
    @objc private func upTotal() {
        //: if currentViewController()?.isKind(of: TalkingWebViewController.self) == true {
        if userRead()?.isKind(of: EqualViewController.self) == true {
            //: let vc = currentViewController() as! TalkingWebViewController
            let vc = userRead() as! EqualViewController
            //: if (vc.urlStr.contains(WebUrlType.RechargeHalfPage.rawValue) || vc.urlStr.contains(WebUrlType.SubscribeAlert.rawValue )) && vc.isRechargeOrSubscribeChangeMatch {
            if vc.urlStr.contains(AutomaticallyCollection.RechargeHalfPage.rawValue) || vc.urlStr.contains(AutomaticallyCollection.SubscribeAlert.rawValue), vc.isRechargeOrSubscribeChangeMatch {
                //: stop_VideoCallTimer()
                numbererval()
                //: vc.dismiss(animated: true)
                vc.dismiss(animated: true)
//                tabberSelete()
            }
        }
    }
}

// MARK: - Load Data

//: extension TalkingVideoInitivCallManager {
extension NameCallManager {
    /// 获取用户主动拨打视频信息
    //: @objc private func req_getVideoInfo() {
    @objc private func addDown() {
        //: let reqModel = TalkingRequestModel.init()
        let reqModel = SafetyMarginReactiveCompatible()
        //: reqModel.requestPath = "mf/freeCall/getVideoInfo"
        reqModel.requestPath = (String(str_sessionGiftName) + String(str_equalValue) + String(str_thumbBackValue))
        //: reqModel.requestType = .GET
        reqModel.requestType = .GET
        //: reqModel.params = ["fromType": "0"]
        reqModel.params = [String(bytes: str_sendValue.reversed(), encoding: .utf8)!: "0"]
        //: reqModel.showErrorStatusBar = false
        reqModel.showErrorStatusBar = false
        //: GJ.startRequest(model: reqModel) { succeed, result, errorModel in
        consLet_listTitle.coloring(model: reqModel) { succeed, result, errorModel in
            /// 用户正在通话中，需要开启x秒后再次请求
            //: if errorModel?.errorCode == 104 {
            if errorModel?.errorCode == 104 {
                //: self.initVideoCallTimer()
                self.mix()
                //: return
                return
            }
            //: guard succeed == true else {
            guard succeed == true else {
                //: self.distroryNotif()
                self.userShow()
                //: self.stop_VideoCallTimer()
                self.numbererval()
                //: return
                return
            }
            //: guard let result = result as? Dictionary<String, Any> else {
            guard let result = result as? [String: Any] else {
                //: return
                return
            }
            //: if let model = TalkingVideoInitivCallModel.deserialize(from: result as? Dictionary) {
            if let model = AstatineMeasurable.deserialize(from: result as? Dictionary) {
                //: self.videoCallModel = model
                self.videoCallModel = model
            }
            /// 预加载
            //: let  player = TalkingVideoPlayerManager.init()
            let player = WoodPlayerManager()
            //: player.setMute(bEnable: true)
            player.setQuantity(bEnable: true)
            //: player.playerWithUrlAndVideoView(url: self.videoCallModel.winVideoUrl, view: UIView.init())
            player.seedWithoutForefront(url: self.videoCallModel.winVideoUrl, view: UIView())

            //: self.videoCallTimerPopView()
            self.selected()
        }
    }

    /** 接口记录用户是否接通或拒绝当前通话
     *  type :3 拒绝 1 接通 2 挂断
     *  duration 接通时长
     */
    //: func req_userOperation(type: Int, duration: Int = 0, stopUid: String = "", completion: @escaping FinishBlock) {
    func fortView(type: Int, duration: Int = 0, stopUid: String = "", completion: @escaping FinishBlock) {
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["videoId"] = videoCallModel.videoId
        dict["videoId"] = videoCallModel.videoId
        //: dict["uid"] = videoCallModel.uid
        dict["uid"] = videoCallModel.uid
        //: dict["type"] = type
        dict["type"] = type
        //: dict["logId"] = videoCallModel.logId
        dict["logId"] = videoCallModel.logId

        //: if duration > 0 {
        if duration > 0 {
            //: dict["duration"] = duration
            dict[(str_showText.replacingOccurrences(of: "value", with: "i") + String(str_callData))] = duration
        }
        //: if stopUid.count > 0 {
        if stopUid.count > 0 {
            //: dict["stopUid"] = stopUid
            dict["stopUid"] = stopUid
        }

        //: let reqModel = TalkingRequestModel.init()
        let reqModel = SafetyMarginReactiveCompatible()
        //: reqModel.requestPath = "mf/freeCall/operation"
        reqModel.requestPath = (String(str_maxName))
        //: reqModel.requestType = .POST
        reqModel.requestType = .POST
        //: reqModel.params = dict
        reqModel.params = dict
        //: GJ.startRequest(model: reqModel) { succeed, result, errorModel in
        consLet_listTitle.coloring(model: reqModel) { succeed, result, errorModel in
            //: completion(succeed, result, errorModel)
            completion(succeed, result, errorModel)
        }
    }
}
