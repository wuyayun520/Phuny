...
fileprivate let str_countText:[UInt8] = [0xe0,0xe7,0xe0,0xfd,0xa1,0xea,0xe6,0xed,0xec,0xfb,0xb3,0xa0,0xa9,0xe1,0xe8,0xfa,0xa9,0xe7,0xe6,0xfd,0xa9,0xeb,0xec,0xec,0xe7,0xa9,0xe0,0xe4,0xf9,0xe5,0xec,0xe4,0xec,0xe7,0xfd,0xec,0xed]

private func inType(top num: UInt8) -> UInt8 {
    return num ^ 137
}

/*: "Not yet~" :*/
fileprivate let str_offData:[Character] = ["N","o","t"," ","y"]
fileprivate let str_colorTitle:[Character] = ["e","t","~"]

/*: "#8C7AFF" :*/
fileprivate let str_mainPointContent:String = "#8C7AFFtype make cell normal"
...
class ShowThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_countText.map{inType(top: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var dataArray: [SickbayModelType] = [] {
        //: didSet {
        didSet {
    ...
    lazy var ranktableView = UITableView(frame: self.bounds, style: .plain).then {
        ...
        $0.register(WineWaiterRoundedCell.self, forCellReuseIdentifier: WineWaiterRoundedCell.className())
        //: $0.register(SocialRankListViewCell.self, forCellReuseIdentifier: SocialRankListViewCell.className())
        $0.register(MagnitudeThen.self, forCellReuseIdentifier: MagnitudeThen.className())
    }
    ...
    lazy var tableHeaderView = BlockView(frame: CGRect(x: 0, y: 0, width: consLet_errorDeviceHeightData, height: actualHeight(h: 220 + 14))).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.index = self.type
        $0.index = self.type
    ...
    lazy var tableBackgrourdView = DeviceView().then {
...
extension ShowThen: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: WineWaiterRoundedCell.className(), for: indexPath) as! WineWaiterRoundedCell
            //: return cell
            return cell
        }
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: MagnitudeThen.className(), for: indexPath) as! MagnitudeThen
        //: let model = dataArray[indexPath.row+3]
        let model = dataArray[indexPath.row + 3]
        //: cell.updateRankCell(model: model, index: indexPath.row+3)
        cell.dataConverterIndex(model: model, index: indexPath.row + 3)
        //: cell.updateIcon(type: self.type)
        cell.infoType(type: self.type)
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: WineWaiterRoundedCell.className()) as! WineWaiterRoundedCell
...
extension ShowThen: JXSegmentedListContainerViewListDelegate {
