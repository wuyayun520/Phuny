...
fileprivate let str_equalName:[UInt8] = [0xe8,0xef,0xe8,0xf5,0xa9,0xe2,0xee,0xe5,0xe4,0xf3,0xbb,0xa8,0xa1,0xe9,0xe0,0xf2,0xa1,0xef,0xee,0xf5,0xa1,0xe3,0xe4,0xe4,0xef,0xa1,0xe8,0xec,0xf1,0xed,0xe4,0xec,0xe4,0xef,0xf5,0xe4,0xe5]

/*: "img_home_shadow" :*/
fileprivate let str_areaName:[Character] = ["i","m","g","_","h","o","m","e","_","s","h","a","d","o","w"]

/*: "icon_match_dislikeBtn_nor" :*/
fileprivate let str_backgroundTitle:String = "icon_background transition normal label"
fileprivate let str_sharedSexMessageValue:String = "h_didata image color"
fileprivate let str_kitValue:[Character] = ["B","t","n","_","n","o","r"]

/*: "icon_match_likeBtn_nor" :*/
fileprivate let str_sendData:[Character] = ["i","c","o","n","_","m","a"]
fileprivate let str_backValue:[Character] = ["t","c","h"]
fileprivate let str_makeValue:[Character] = ["_","l","i","k","e","B","t","n","_","n","o","r"]
...
protocol WeltanschauungDataSource: NSObjectProtocol {
    ///
    //: func numberOfCountInCardView(cardView: TalkingClubCardView) -> Int
    func clickCardView(cardView: BlockObjectProtocol) -> Int
...
@objc protocol WhiteObjectProtocol: NSObjectProtocol {
    ///
    ...
    @objc optional func cellRoom(cardView: BlockObjectProtocol, cardViewCell: SequenceErrorViewCell, forRowAtIndex: Int, direction: Int, isGesture: Bool)

    //: @objc optional func didRemoveLastCell(cardViewCell: TalkingClubCardViewCell, forRowAtIndex: Int)
    @objc optional func footballPlayerUp(cardViewCell: SequenceErrorViewCell, forRowAtIndex: Int)

    //: @objc optional func didDisplayCell(cardViewCell: TalkingClubCardViewCell, forRowAtIndex: Int)
    @objc optional func barSize(cardViewCell: SequenceErrorViewCell, forRowAtIndex: Int)

    //: @objc optional func didMoveCell(cardView: TalkingClubCardView, cardViewCell: TalkingClubCardViewCell, forMovePoint: CGPoint)
    @objc optional func countColor(cardView: BlockObjectProtocol, cardViewCell: SequenceErrorViewCell, forMovePoint: CGPoint)

    //: @objc optional func beginMoveCell(cardView: TalkingClubCardView, cardViewCell: TalkingClubCardViewCell)
    @objc optional func enumerationCell(cardView: BlockObjectProtocol, cardViewCell: SequenceErrorViewCell)
}
...
class BlockObjectProtocol: UIView {
    ...
    var maxRemoveDistance = consLet_errorDeviceHeightData / 4.0 /// 最大移除距离(默认屏幕的1/4)
    //: var reusableCells = [TalkingClubCardViewCell]()
    var reusableCells = [SequenceErrorViewCell]() /// 重用卡片数组
    //: var isAlpha = true
    ...
    var dataSource: WeltanschauungDataSource? /// 数据源
    //: var delegate: TalkingClubCardViewDelegate?
    var delegate: WhiteObjectProtocol?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_equalName.map{$0^129}, encoding: .utf8)!)
    }
    ...
    private lazy var leftbackImageView: UIImageView = {
        ...
        imag.contentMode = .scaleAspectFill
        //: imag.image = UIImage.nearTap(name: "img_home_shadow")
        imag.image = UIImage.nearTap(name: (String(str_areaName)))
    ...
    private lazy var disLikeImage: UIImageView = {
        //: let img = UIImageView()
        let img = UIImageView()
        //: img.image = UIImage.nearTap(name: "icon_match_dislikeBtn_nor")
        img.image = UIImage.nearTap(name: (String(str_backgroundTitle.prefix(5)) + "matc" + String(str_sharedSexMessageValue.prefix(4)) + "slike" + String(str_kitValue)))
        //: img.isHidden = true
        img.isHidden = true
        //: img.alpha = 0.5
        img.alpha = 0.5
    ...
    private lazy var likeImage: UIImageView = {
        //: let img = UIImageView()
        let img = UIImageView()
        //: img.image = UIImage.nearTap(name: "icon_match_likeBtn_nor")
        img.image = UIImage.nearTap(name: (String(str_sendData) + String(str_backValue) + String(str_makeValue)))
        //: img.isHidden = true
        img.isHidden = true
        //: img.alpha = 0.5
        img.alpha = 0.5
...
extension BlockObjectProtocol {
    ...
    func pushDataEqual(anim: Bool) {
        ...
        let maxCount = dataSource?.clickCardView(cardView: self) ?? 0
    ...
    func inundate(index: Int) {
        //: let cell = dataSource?.cardViewCellForRowAtIndex(cardView: self, index: index)
        let cell = dataSource?.someAttribute(cardView: self, index: index)
...
//        let showCount = Double(self.visibleCount - 1)
        ...
        containerView.insertSubview(cell ?? SequenceErrorViewCell(), at: 0)
    ...
    func cellsAnim(anim: Bool) {
        ...
        for i in 0 ... count - 1 {
            ...
            let cell: SequenceErrorViewCell = self.visibleCells[i] as! SequenceErrorViewCell
            ...
            if isVisbleLast {
                ...
                self.delegate?.barSize?(cardViewCell: cell, forRowAtIndex: self.currentIndex - i)
            }

            //: if anim {
            if anim {
                //: updateConstraintsCell(cell: cell, transform: transform)
                flushOf(cell: cell, transform: transform)
                //: } else {
            } else {
    ...
    func flushOf(cell: SequenceErrorViewCell, transform: CGAffineTransform) {
    ...
    func roundWithClick(inde: Int) -> Int {
    ...
    func substitutionClassIdentifier(cellClass: AnyClass, forellIdentifier: String) {
    ...
    func addIdentifier(identifier: String) -> SequenceErrorViewCell? {
        ...
        if self.cellClass != nil {
            ...
            let cellclass = self.cellClass as! ResourceFailRecognizerDelegate.Type
            //: let cell: TalkingClubCardItemCell = cellclass.init(reuseIdentifier: identifier )
            let cell: ResourceFailRecognizerDelegate = cellclass.init(reuseIdentifier: identifier)
    ...
    func constraintTop(direction: DiscourseBasicType) {
        //: if self.visibleCells.count == 0 { return }
        if self.visibleCells.count == 0 { return }
        //: let topcell = self.visibleCells.last as? TalkingClubCardViewCell
        let topcell = self.visibleCells.last as? SequenceErrorViewCell
        //: topcell?.removeFromSuperviewSwipe(direction: direction)
        topcell?.swipeDirection(direction: direction)
    }
...
/// SerrationObjectProtocol
//: extension TalkingClubCardView: TalkingClubCardViewCellDelegete {
extension BlockObjectProtocol: SerrationObjectProtocol {
    ...
    func upGesture(cell: SequenceErrorViewCell, direction: DiscourseBasicType, isGesture: Bool) {
        ...
        self.delegate?.cellRoom?(cardView: self, cardViewCell: cell, forRowAtIndex: self.currentFirstIndex - 1, direction: direction.rawValue, isGesture: isGesture)
        ...
        if self.visibleCells.count == 0 { // 数据看完了
            ...
            self.delegate?.footballPlayerUp?(cardViewCell: cell, forRowAtIndex: self.currentIndex)
            //: return
            return
        }

        //: let count = self.dataSource?.numberOfCountInCardView(cardView: self) ?? 0
        let count = self.dataSource?.clickCardView(cardView: self) ?? 0

        // 当前数据源还有数据 继续创建cell
        //: if (self.currentIndex < count - 1) {
        if self.currentIndex < count - 1 {
            //: createCardViewCell(index: self.currentIndex + 1)
            inundate(index: self.currentIndex + 1)
        }
        // 更新布局
    ...
    func exhaustScreen(cell: SequenceErrorViewCell, forMovePoint: CGPoint, direction: DiscourseBasicType) {
        ...
        self.delegate?.countColor?(cardView: self, cardViewCell: cell, forMovePoint: forMovePoint)
        //: if  direction == .Left{
        if direction == .Left {
    ...
    func smearCell(cell _: SequenceErrorViewCell) {
        //: disLikeImage.isHidden = true
        disLikeImage.isHidden = true
        //: likeImage.isHidden = true
        likeImage.isHidden = true
    ...
    func colorEqual(cell: SequenceErrorViewCell) {
        ...
        self.delegate?.enumerationCell?(cardView: self, cardViewCell: cell)
    }
}
