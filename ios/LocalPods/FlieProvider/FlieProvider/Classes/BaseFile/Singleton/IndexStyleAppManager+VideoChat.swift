...
/*: "type" :*/
fileprivate let str_rowTextName:[Character] = ["t","y","p","e"]

/*: "You are currently on a call, please try again later" :*/
fileprivate let str_frameName:[UInt8] = [0xe5,0xd3,0xc9,0x9c,0xdd,0xce,0xd9,0x9c,0xdf,0xc9,0xce,0xce,0xd9,0xd2,0xc8,0xd0,0xc5,0x9c,0xd3,0xd2,0x9c,0xdd,0x9c,0xdf,0xdd,0xd0,0xd0,0x90,0x9c,0xcc,0xd0,0xd9,0xdd,0xcf,0xd9,0x9c,0xc8,0xce,0xc5,0x9c,0xdd,0xdb,0xdd,0xd5,0xd2,0x9c,0xd0,0xdd,0xc8,0xd9,0xce]

private func liveNameMobile(mini num: UInt8) -> UInt8 {
    return num ^ 188
}

/*: "onCallResume" :*/
fileprivate let str_loadValue:String = "onCalpopular data path name"
...
// __CLOSE_PRINT__
//
//  IndexStyleAppManager+VideoChat.swift
//  AbroadTalking
//
...
extension IndexStyleAppManager {
    ...
    func componentView(info: [String: Any], situation: MagnitudeensityChatSituation = .Start) {
        //: let json = JSON(info)
        let json = JSON(info)
        ...
        if type == 0 { // 视频
            ...
            (vc as! MetronymicThen).chatModel = PastChatModel.videoRemove(info: info, situation: situation)
            //: } else {
        } else { // 音频
            //: vc = TalkingVoiceChatViewController()
            vc = NameManagerViewDelegate()
            //: (vc as! TalkingVoiceChatViewController).chatModel = TalkingVideoChatModel.getVideoModel(info: info, situation: situation)
            (vc as! NameManagerViewDelegate).chatModel = PastChatModel.videoRemove(info: info, situation: situation)
        }

        //: self.tryToPushTalkRoom(roomVC: vc!, isVideoTalk: (type == 0 ? true : false), resumeTalk: false, resumeData: [:])
        self.copernicanSystem(roomVC: vc!, isVideoTalk: type == 0 ? true : false, resumeTalk: false, resumeData: [:])
    }

    /// 接收到视频通话
    /// - Parameter info: 数据
    //: func recieve1v1TalkRequest(info: [String: Any]) {
    func equalColor(info: [String: Any]) {
        //: handle1v1AudioRequest(info: info, resumeTalk: false)
        communicationRecord(info: info, resumeTalk: false)
    }

    /// 恢复视频通话
    /// - Parameter info: 数据
    //: func recieve1v1TalkResumeRequest(info: [String: Any]) {
    func courtInFormatting(info: [String: Any]) {
        //: handle1v1AudioRequest(info: info, resumeTalk: true)
        communicationRecord(info: info, resumeTalk: true)
    }
}
...
extension IndexStyleAppManager {
    ...
    private func communicationRecord(info: [String: Any], resumeTalk: Bool) {
        //: if !resumeTalk && TalkingSocketManager.shared.isTalking {
        if !resumeTalk && JewishCalendarMonthThen.shared.isTalking {
            //: self.func__showStatusBarErrorMsg(showMsg: "You are currently on a call, please try again later".localized)
            self.noblesseExtra(showMsg: String(bytes: str_frameName.map{liveNameMobile(mini: $0)}, encoding: .utf8)!.localized)
            //: return
            return
        }
        ...
        if type == 0 { // 视频通话
            ...
            (vc as! MetronymicThen).chatModel = PastChatModel.videoRemove(info: info, situation: resumeTalk ? .Resume : .Recieved)
            //: } else {
        } else { // 语音通话
            //: vc = TalkingVoiceChatViewController()
            vc = NameManagerViewDelegate()
            ...
            (vc as! NameManagerViewDelegate).chatModel = PastChatModel.videoRemove(info: info, situation: resumeTalk ? .Resume : .Recieved)
        }

        //: let callResume = json["onCallResume"].stringValue
        let callResume = json[(String(str_loadValue.prefix(5)) + "lResume")].stringValue
        ...
        if resumeTalk, callResume.count > 0, callResume == "1" { // 恢复通话
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1) {
                ...
                self.copernicanSystem(roomVC: vc!, isVideoTalk: type == 0 ? true : false, resumeTalk: resumeTalk, resumeData: info)
            }
            //: } else {
        } else { // 构建音视频通话
            //: self.tryToPushTalkRoom(roomVC: vc!, isVideoTalk: (type == 0 ? true : false), resumeTalk: resumeTalk, resumeData: info)
    ...
    private func copernicanSystem(roomVC: UIViewController, isVideoTalk: Bool, resumeTalk: Bool, resumeData: [String: Any]) {
        //: guard var currentVc = currentViewController() else { return }
        guard var currentVc = userRead() else { return }
        ...
        if currentVc.isKind(of: UINavigationController.self) {
            ...
            //: let rootVc = OverdoReactiveCompatible.getWindow().rootViewController as! UITabBarController
            let rootVc = OverdoReactiveCompatible.outMax().rootViewController as! UITabBarController
        ...
        if !isVideoTalk {
            ...
            talkClass = NSStringFromClass(NameManagerViewDelegate.self)
            //: } else {
        } else {
            //: talkClass = NSStringFromClass(TalkingVideoChatViewController.self)
            talkClass = NSStringFromClass(MetronymicThen.self)
        }
        //: var chatIndex = -1
        var chatIndex = -1
        //: var index = -1
        ...
        if isVideoTalk {
            ...
            var currentChatVc: MetronymicThen?
            //: let pushChatVc = roomVC as! TalkingVideoChatViewController
            let pushChatVc = roomVC as! MetronymicThen

            //: if chatIndex >= 0 {
            if chatIndex >= 0 {
                //: currentChatVc = navVc?.viewControllers[chatIndex] as? TalkingVideoChatViewController
                currentChatVc = navVc?.viewControllers[chatIndex] as? MetronymicThen
            }
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    ...
                    currentChatVc?.metadata(resumeData: resumeData)
                }
                //: return
                return
            }
            ...
            if navVc!.isKind(of: NameModelNavigationController.self) {
                //: let nav = navVc as! TalkingNavigationController
                let nav = navVc as! NameModelNavigationController
            ...
            if topVc != nil, topVc!.isKind(of: AutomaticallyErrorDelegate.self) {
            ...
            var currentChatVc: NameManagerViewDelegate?
            //: let pushChatVc = roomVC as! TalkingVoiceChatViewController
            let pushChatVc = roomVC as! NameManagerViewDelegate

            //: if chatIndex >= 0 {
            if chatIndex >= 0 {
                //: currentChatVc = navVc?.viewControllers[chatIndex] as? TalkingVoiceChatViewController
                currentChatVc = navVc?.viewControllers[chatIndex] as? NameManagerViewDelegate
            }
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    //: currentChatVc?.updateInfoViaResumeTalkData(resumeData: resumeData)
                    currentChatVc?.upwardVoice(resumeData: resumeData)
                }
                //: return
                return
            }
