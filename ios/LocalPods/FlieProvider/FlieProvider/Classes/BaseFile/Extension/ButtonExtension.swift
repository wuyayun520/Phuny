...
/*: "recharge" :*/
fileprivate let str_flexibleText:String = "all"
fileprivate let str_onlineToText:String = "keyharge"

/*: "recharge_ :*/
fileprivate let str_succeedData:[Character] = ["r","e","c","h","a","r","g","e","_"]

/*: ".png" :*/
fileprivate let str_moreName:[Character] = [".","p","n","g"]

/*: "_ :*/
fileprivate let str_ofValue:String = "to"

/*: .png" :*/
fileprivate let str_bottomValue:String = "equal repeat index.png"

/*: ".jpg" :*/
fileprivate let str_listenValue:[Character] = [".","j","p","g"]

/*: .jpg" :*/
fileprivate let str_mainDataName:String = ".jpgtry count var"
...
enum StoreImageAlignment: NSInteger {
    //: case left = 0
    case left = 0
    //: case top
    case top
...
class LabelPositionReactiveCompatible: UIButton {
    ...
    var imageAlignment: StoreImageAlignment = .left
...
extension UIButton {
    ...
    func indexPhoto(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        used(interval: interval, topOffset: 0)
    }
    ...
    func used(interval: CGFloat, topOffset: CGFloat) {
    ...
    func fileData(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func fullMoon(color: UIColor, forState: UIControl.State) {
    ...
    func net(urlStr: String,
             //: placeImg: UIImage? = UIImage.placeImgSquare(),
             placeImg: UIImage? = UIImage.requestInfo(),
             ...
             loadFinish: ((_ isSucceed: Bool) -> Void)? = nil)
    {
        ...
        let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: consLet_netTitle)
    ...
    func arrange(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((str_flexibleText.replacingOccurrences(of: "all", with: "r") + str_onlineToText.replacingOccurrences(of: "key", with: "ec"))) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (str_flexibleText.replacingOccurrences(of: "all", with: "r") + str_onlineToText.replacingOccurrences(of: "key", with: "ec")), with: (String(str_succeedData)) + "\(consLet_packageName)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.net(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains(".png") {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(consLet_packageName).png")

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains(".jpg") {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(consLet_packageName).jpg")
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.net(urlStr: newUrl, placeImg: nil)
    }
