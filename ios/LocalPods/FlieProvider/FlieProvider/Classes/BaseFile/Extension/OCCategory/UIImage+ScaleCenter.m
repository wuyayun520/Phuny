...
#import "UIImage+ScaleCenter.h"

//: @implementation UIImage (Compress)
@implementation UIImage (ScaleCenter)

//: - (NSData *)compressedImageData {
- (NSData *)number {

    //: UIImage *image = self.wcTimelineCompress;
    UIImage *image = self.outsideIcon;
...
- (UIImage *)cogitationAuditoryImage {
    ...
    return [UIImage imageWithData:[self number]];
}
...
- (UIImage *)lapsing:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self fromSize:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self withObject:size];
...
- (CGSize)fromSize:(Boolean)isSession {
...
+ (NSData *)key:(UIImage *)image handleCover:(NSInteger)fileSize{
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG to:scalesize localValue:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG render:(fileSize*1024)];
    }
...
- (UIImage*)largish:(CGSize)targetSize{
...
- (UIImage *)to:(CGSize)targetSize localValue:(BOOL)isScale{
...
- (NSData *)render:(NSInteger)fileSize{
...
- (UIImage *)withObject:(CGSize)newSize {
