...
/*: "add_header_cookies" :*/
fileprivate let str_succeedValue:[Character] = ["a","d","d","_","h","e","a"]
fileprivate let str_cellName:[Character] = ["d","e","r","_","c","o","o","k","i","e","s"]
...
public extension UIImageView {
    ...
    @objc func underPause(urlStr: String,
    ...
    internal func cling(urlStr: String,
                        //: placeImg: UIImage = UIImage.placeImgSquare(),
                        placeImg: UIImage = UIImage.requestInfo(),
    ...
    @objc func settleDownInsideMentalRepresentation(urlStr: String) {
        //: let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: consLet_netTitle)
        //: self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.giftPlaceHolderImage())
        self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.totalerval())
    }
    ...
    @objc func setBlockClick(urlStr: String, completion: ((_ image: UIImage?) -> Void)? = nil) {
        // 允许请求携带cookie
        ...
        if var urlComponents = URLComponents(string: urlStr),
           ...
           let currURL = URL(string: TalkingBenderTotalensityRequestThen.share.serverUrlStr)
        {
        ...
        let downloader = ImageDownloader(name: (String(str_succeedValue) + String(str_cellName)))
        ...
        self.kf.setImage(with: URL(string: newImgUrl), placeholder: UIImage.requestInfo(), options: optionsInfo) { result in
            //: if completion != nil {
            if completion != nil {
                //: let image = try? result.get().image
                let image = try? result.get().image
