...
/*: "http://stat. :*/
fileprivate let str_pickLabContent:[Character] = ["h","t","t"]
fileprivate let str_jumpRunData:String = "p://scolor invite"

/*: .com" :*/
fileprivate let str_reportName:[Character] = [".","c","o","m"]

/*: "http://" :*/
fileprivate let str_itemTitle:String = "hless"
fileprivate let str_pathContent:String = "tp://image self hidden let"

/*: "https://" :*/
fileprivate let str_centerName:String = "model number with nohttps://"

/*: "http://m. :*/
fileprivate let str_connectionName:String = "http://round let"
fileprivate let str_underValue:[Character] = ["m","."]

/*: "http:// :*/
fileprivate let str_arrayData:[Character] = ["h"]
fileprivate let str_listValue:String = "ttp://line video style female"

/*: "-1" :*/
fileprivate let str_yourName:[Character] = ["-","1"]

/*: "3,90" :*/
fileprivate let str_videoValue:[Character] = ["3",",","9","0"]

/*: "1990-01-01" :*/
fileprivate let str_enableData:String = "1model0"
fileprivate let str_faceOnData:[Character] = ["-","0","1","-","0","1"]

/*: "MM-dd-yyyy" :*/
fileprivate let str_appStateData:[Character] = ["M","M"]
fileprivate let str_progressInputText:[Character] = ["-","d","d","-","y","y","y","y"]

/*: "id" :*/
fileprivate let str_voiceData:[Character] = ["i","d"]
...
class SafetyMarginReactiveCompatible: NSObject {
    ...
    override init() {
        ...
        self.requestServer = TalkingBenderTotalensityRequestThen.share.serverUrlStr
        //: self.showErrorStatusBar = true
        self.showErrorStatusBar = true
    }

    //: func paramsContainBinaryObject() -> Bool {
    func fromObject() -> Bool {
...
struct BlockVideoBaseResponse: HandyJSON {
    //: var errno: Int!
...
public struct SpotErrorResponse {
    //: let errorCode: Int
    let errorCode: Int
    //: let errorMsg: String
    let errorMsg: String
...
struct LabelHandyJSON: HandyJSON {
    ...
    var CSConfig = ConfigTransformable() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (String(str_pickLabContent) + String(str_jumpRunData.prefix(5)) + "tat.") + "\(consLet_netTitle).com"
            //: } else {
        } else {
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return "http://m.\(consLet_netTitle).com"
            //: } else {
        } else {
...
public class IsoclinalReactiveCompatible: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [IndexTransformable] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [IndexTransformable] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [IndexTransformable] = [] // 语音收费设置
    ...
    var vipSetting = [GiftMeasurable]() // vip皮肤
    ...
    var tabPageBanner = [LastModelType]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = ExpressMeasurable() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class DominoUserModel: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [ErrorEquatable]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [ErrorEquatable]?
    //: var partner: String?
    var partner: String?
    //: var full: Int?
    var full: Int?
    ...
    var gallery: [ProposalTransformable]?
    //: var status: Int = -1
    ...
    var residualInfo = [WhiteMeasurable]() // 套餐数量

    //: func updateResidualNum(type: Int, num: Int) {
    func hitVariant(type: Int, num: Int) {
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.combine(self.birthday ?? (str_enableData.replacingOccurrences(of: "model", with: "99") + String(str_faceOnData)), dateFormat: (String(str_appStateData) + String(str_progressInputText)))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.leapYear().year else { return "0" }
...
struct WhiteMeasurable: HandyJSON {
    //: var type = 0
...
struct ProposalTransformable: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct ErrorEquatable: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func selectedTo(compareTo: ErrorEquatable) -> Bool {
        //: return
        return
...
struct UserBlockHandyJSON: HandyJSON {
    //: var aboutMe: Array<UserTagTypeModel>!
    var aboutMe: [NameModelType]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [NameModelType]!
}
...
struct NameModelType: HandyJSON {
    ...
    var list: [ErrorEquatable]!
}

/// 官方客服提示消息模型
//: struct CSConfigModel: HandyJSON {
struct ConfigTransformable: HandyJSON {
...
struct GiftMeasurable: HandyJSON {
...
struct ExpressMeasurable: HandyJSON {
    //: var sendImg = 10
    var sendImg = 10 // 发送图片
    //: var sendVideo = 10
