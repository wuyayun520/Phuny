...
fileprivate let str_labTextValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
typealias SlidingHandler = (_ direction: OnlySlideDirection) -> Void
...
class BlockViewDelegate: UIView {
    //: var cropStart: CGFloat {
    var cropStart: CGFloat {
        //: let offset = overlay.startOffset - (LeftMargin - self.contentView.contentOffset.x)
        let offset = overlay.startOffset - (consLet_appErrorText - self.contentView.contentOffset.x)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_labTextValue.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIScrollView = {
        ...
        while time <= self.duration {
            ...
            let image = self.holderAttribute(point: time)
            //: imv.image = image
            imv.image = image
    ...
    lazy var overlay: AutomaticallyColorReactiveCompatible = {
        ...
        let overlay = AutomaticallyColorReactiveCompatible(frame: self.bounds, maxCropLen: maxWidth, minCropLen: minWidth)
...
extension BlockViewDelegate {
    ...
    private func holderAttribute(point: CGFloat) -> UIImage {
...
extension BlockViewDelegate: UIScrollViewDelegate {
    //: func scrollViewDidScroll(_ scrollView: UIScrollView) {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //: var offset = scrollView.contentOffset
        var offset = scrollView.contentOffset
        //: let minx = LeftMargin - overlay.startOffset
        let minx = consLet_appErrorText - overlay.startOffset
    ...
    func scrollViewWillBeginDragging(_: UIScrollView) {
        ...
        self.overlay.nameShow(isShow: false)
    }
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        self.overlay.nameShow(isShow: true)
    }
...
extension BlockViewDelegate {
    ...
    private func atomicNumber85() {
        ...
        self.contentView.snp.makeConstraints { make in
            ...
            make.leading.equalToSuperview().offset(consLet_appErrorText)
