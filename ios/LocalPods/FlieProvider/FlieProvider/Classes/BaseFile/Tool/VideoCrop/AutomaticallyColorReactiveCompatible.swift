...
fileprivate let str_equalName:[UInt8] = [0x4d,0x4a,0x4d,0x50,0xc,0x47,0x4b,0x40,0x41,0x56,0x1e,0xd,0x4,0x4c,0x45,0x57,0x4,0x4a,0x4b,0x50,0x4,0x46,0x41,0x41,0x4a,0x4,0x4d,0x49,0x54,0x48,0x41,0x49,0x41,0x4a,0x50,0x41,0x40]

private func selfView(raw num: UInt8) -> UInt8 {
    return num ^ 36
}

/*: "FFD855" :*/
fileprivate let str_loadData:String = "ffd855"
...
// __CLOSE_PRINT__
//
//  AutomaticallyColorReactiveCompatible.swift
//  AbroadTalking
//
...
class AutomaticallyColorReactiveCompatible: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - consLet_name
                ...
                self.indite(startX: self.videoStartX)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_equalName.map{selfView(raw: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: consLet_appErrorText, y: 0, width: consLet_name, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - consLet_name, y: 0, width: consLet_name, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.computeImage(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: consLet_appErrorText, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.computeImage(0, 0, 0, 0.6)
...
extension AutomaticallyColorReactiveCompatible {
    ...
    func indite(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - consLet_name,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - consLet_name)
    }
}
...
extension AutomaticallyColorReactiveCompatible {
    ...
    @objc func assemblagePost(gesture: UIPanGestureRecognizer) {
        ...
        case .ended:
            ...
            self.indite(startX: videoStartX)
    ...
    @objc func countIndexGift(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension AutomaticallyColorReactiveCompatible {
    ...
    private func eraseOn() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(assemblagePost(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(countIndexGift(gesture:)))
    ...
    func flushQuantity(_ xposition: CGFloat) {
