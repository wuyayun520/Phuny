...
/*: "Finish" :*/
fileprivate let str_addTitle:String = "like group index infoFinish"

/*: "Cancel" :*/
fileprivate let str_countLeadingValue:[Character] = ["C","a","n","c","e"]
fileprivate let str_norBagValue:[Character] = ["l"]

/*: "btn_photo_choice_pre" :*/
fileprivate let str_sizeName:String = "if forbtn_photo"
fileprivate let str_rowValue:String = "_chmake self self"
fileprivate let str_roundValue:[Character] = ["o","i","c","e","_","p","r","e"]

/*: "btn_photo_add_nor" :*/
fileprivate let str_femaleValue:String = "let view videobtn_phot"
fileprivate let str_handleData:String = "o_adtouch if view lab to"

/*: "( :*/
fileprivate let str_localGiftContent:String = "session"

/*: )" :*/
fileprivate let str_currentData:String = ")"

/*: "/tmp" :*/
fileprivate let str_meValue:[Character] = ["/","t","m","p"]

/*: "Export session failed" :*/
fileprivate let str_lengthData:String = "size from random manager priceExport"
fileprivate let str_sharedActionClothesTitle:String = "view self any varsion "
fileprivate let str_lightData:[Character] = ["f"]
fileprivate let str_dateValue:[Character] = ["a","i","l","e","d"]

/*: "Export canceled" :*/
fileprivate let str_imageName:String = "Expotext equal to equal"
fileprivate let str_timeName:String = "nceshared"

/*: "Successful!" :*/
fileprivate let str_labEnableTargetName:String = "Successtop target"
fileprivate let str_normalName:[Character] = ["f","u","l","!"]

/*: "Unsupported video formats" :*/
fileprivate let str_logAnswerValue:String = "Unsuppcolor more let"
fileprivate let str_indexText:String = " videwhite text title"
fileprivate let str_styleValue:String = "flow"

/*: "yyyy-MM-dd-HH:mm:ss-SSS" :*/
fileprivate let str_resourceValue:String = "case phone useryyyy-"
fileprivate let str_floorContent:String = "HH:mm:schoice device"
fileprivate let str_callValue:[Character] = ["s","-","S","S","S"]

/*: "/tmp/video- :*/
fileprivate let str_popData:String = "/tmp/vvar let"
fileprivate let str_statusText:String = "min view to returnideo-"

/*: - :*/
fileprivate let str_viewData:String = "-"

/*: .mp4" :*/
fileprivate let str_styleName:String = ".mp4guard show model"
...
class MightHaveBeenThen: NSObject {
    ...
    class func sendLocal(allowImgCrop: Bool = false, needCircleCrop: Bool = false, maxCount: Int, allowPhoto: Bool, allowVideo: Bool) -> TZImagePickerController {
...
//        imagePicker?.preferredLanguage = "en"
        ...
        if allowImgCrop {
            ...
            var height = consLet_errorDeviceHeightData * 488 / 375
            //: if needCircleCrop == true {
            if needCircleCrop == true { // 切圆图
                //: height = ScreenWidth
                height = consLet_errorDeviceHeightData
            }
            //: imagePicker?.cropRect = CGRect(x: 0, y: (ScreenHeight-height)/2, width: ScreenWidth, height: height)
            imagePicker?.cropRect = CGRect(x: 0, y: (consLet_sessionName - height) / 2, width: consLet_errorDeviceHeightData, height: height)
        }
        //: if allowVideo {
        if allowVideo {
        ...
        //: imagePicker?.barItemTextFont = UIFont.loadEnable(type: .Regular, fontSize: 15)
        imagePicker?.barItemTextFont = UIFont.loadEnable(type: .Regular, fontSize: 15)
        //: imagePicker?.barItemTextColor = UIColor.appValueColor()
        imagePicker?.barItemTextColor = UIColor.frontColor()
        ...
        imagePicker?.naviBgColor = UIColor.white
        //: imagePicker?.naviTitleFont = UIFont.loadEnable(type: .Medium, fontSize: 17)
        imagePicker?.naviTitleFont = UIFont.loadEnable(type: .Medium, fontSize: 17)
        //: imagePicker?.naviTitleColor = UIColor.appTitleColor()
        imagePicker?.naviTitleColor = UIColor.appNameVisualProperty()
        ...
        imagePicker?.iconThemeColor = UIColor.labelSumervalMedium()

        //: imagePicker?.allowPickingOriginalPhoto = false
        imagePicker?.allowPickingOriginalPhoto = false
        //: imagePicker?.photoSelImage = UIImage.nearTap(name: "btn_photo_choice_pre")
        imagePicker?.photoSelImage = UIImage.nearTap(name: (String(str_sizeName.suffix(9)) + String(str_rowValue.prefix(3)) + String(str_roundValue)))
        //: imagePicker?.takePictureImage = UIImage.nearTap(name: "btn_photo_add_nor")
        imagePicker?.takePictureImage = UIImage.nearTap(name: (String(str_femaleValue.suffix(8)) + String(str_handleData.prefix(4)) + "d_nor"))
        ...
        imagePicker?.photoPickerPageDidRefreshStateBlock = { (_: UICollectionView?,
                ...
                previewButton?.isHidden = true
                //: doneButton?.titleLabel?.font = UIFont.loadEnable(type: .Medium, fontSize: 14)
                doneButton?.titleLabel?.font = UIFont.loadEnable(type: .Medium, fontSize: 14)
        ...
        imagePicker?.photoPickerPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                doneButton?.backgroundColor = UIColor.labelSumervalMedium()
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
        ...
        imagePicker?.photoPreviewPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                selectButton?.frame = CGRect(x: consLet_errorDeviceHeightData - 5 - (selectButton?.frame.size.width)!, y: (selectButton?.frame.origin.y)!, width: (selectButton?.frame.size.width)!, height: (selectButton?.frame.size.height)!)
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
    ...
    class func makeCompletion(asset: PHAsset?, completion: @escaping (_ filePath: URL?) -> Void) {
            ...
            if asset!.isKind(of: AVURLAsset.self) {
                ...
                exportSession?.exportAsynchronously(completionHandler: {
                    ...
                    case .failed:
                        ...
                        printLog(message: (String(str_lengthData.suffix(6)) + " ses" + String(str_sharedActionClothesTitle.suffix(5)) + String(str_lightData) + String(str_dateValue)))
                    //: case .cancelled:
                    case .cancelled:
                        //: printLog(message: "Export canceled")
                        printLog(message: (String(str_imageName.prefix(4)) + "rt ca" + str_timeName.replacingOccurrences(of: "share", with: "le")))
                    //: case .completed:
                    case .completed:
                        //: printLog(message: "Successful!")
                        printLog(message: (String(str_labEnableTargetName.prefix(7)) + String(str_normalName)))
                        //: DispatchQueue.main.async {
                        DispatchQueue.main.async {
                ...
                printLog(message: (String(str_logAnswerValue.prefix(6)) + "orted" + String(str_indexText.prefix(5)) + "o format" + str_styleValue.replacingOccurrences(of: "flow", with: "s")))
            }
        }
    }
    ...
    private class func needClose() -> String {
        //: let formater = DateFormatter()
        let formater = DateFormatter()
        //: formater.dateFormat = "yyyy-MM-dd-HH:mm:ss-SSS"
        formater.dateFormat = (String(str_resourceValue.suffix(5)) + "MM-dd-" + String(str_floorContent.prefix(7)) + String(str_callValue))
        //: let outputPath = NSHomeDirectory().appending("/tmp/video-\(formater.string(from: Date()))-\(Int.random(in: 0...10000000)).mp4")
        let outputPath = NSHomeDirectory().appending((String(str_popData.prefix(6)) + String(str_statusText.suffix(5))) + "\(formater.string(from: Date()))-\(Int.random(in: 0 ... 10_000_000)).mp4")
        //: return outputPath
        return outputPath
    }
}
