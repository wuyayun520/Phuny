...
/*: / :*/
fileprivate let str_titleName:String = "icon"

/*: "\n" :*/
fileprivate let str_modelData:String = "\n"
...
struct PresentationLabelOrigin {
    //: let x =  5.0
    let x = 5.0
    //: let y =  5.0
    let y = 5.0
...
struct InheritanceContainerInset {
    //: let top: CGFloat = 5.0
    let top: CGFloat = 5.0
...
class VideoReactiveCompatible: UIView {
    ...
    convenience init(placeholder: String?, placeholderColor: UIColor?, frame: CGRect) {
        ...
        infoColor(placeholder: placeholder, placeholderColor: placeholderColor)
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        infoColor(placeholder: nil, placeholderColor: nil)
    }
}
...
extension VideoReactiveCompatible {
    ...
    private func infoColor(placeholder: String?, placeholderColor: UIColor?) {
        ...
        plaleLabel.snp.remakeConstraints { make in
            ...
            make.leading.equalTo(PresentationLabelOrigin().x)
            //: make.top.equalTo(PlaceholderLabelOrigin().y)
            make.top.equalTo(PresentationLabelOrigin().y)
            //: make.trailing.equalTo(-PlaceholderLabelOrigin().x)
            make.trailing.equalTo(-PresentationLabelOrigin().x)
        }
        ...
        placeholdertextView.textContainerInset = UIEdgeInsets(top: InheritanceContainerInset().top, left: InheritanceContainerInset().left, bottom: InheritanceContainerInset().bottom, right: InheritanceContainerInset().right)
...
extension VideoReactiveCompatible: UITextViewDelegate {
    //: func textViewDidChange(_ textView: UITextView) {
    func textViewDidChange(_ textView: UITextView) {
        //: checkShowHiddenPlaceholder()
        selectHiddenPlaceholderShow()
...
extension VideoReactiveCompatible {
    ...
    private func selectHiddenPlaceholderShow() {
