...
/*: "Select a date" :*/
fileprivate let str_dataMakeText:[Character] = ["S","e","l","e","c","t"]
fileprivate let str_sceneTitle:String = "party equal a date"

/*: M" :*/
fileprivate let str_colorModelRowContent:String = "max"

/*: D" :*/
fileprivate let str_pathName:[Character] = ["D"]

/*: Y" :*/
fileprivate let str_centerText:String = "model"
...
protocol StyleFailViewDelegate: NSObjectProtocol {
...
class HalfReactiveCompatible: PostView, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    weak var delegate: StyleFailViewDelegate?

    //: override func initPickView() {
    override func anyModel() {
        //: super.initPickView()
        super.anyModel()

        //: titleString = "Select a date".localized
        titleString = (String(str_dataMakeText) + String(str_sceneTitle.suffix(7))).localized
        ...
        managerDay(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func managerDay(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func opticalLens(button: UIButton) {
        //: super.confirmClick(button: button)
        super.opticalLens(button: button)
        //: refreshPickViewData()
        self.year()
        ...
        if let day = selectDay {
            ...
            delegate?.cornerDay(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.cornerDay(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return lease(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func lease(year: Int, month: Int) -> Int {
