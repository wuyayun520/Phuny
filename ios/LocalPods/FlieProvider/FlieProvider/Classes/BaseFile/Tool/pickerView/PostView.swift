...
fileprivate let str_quoteMoreName:[UInt8] = [0xa,0xd,0xa,0x17,0x4b,0x0,0xc,0x7,0x6,0x11,0x59,0x4a,0x43,0xb,0x2,0x10,0x43,0xd,0xc,0x17,0x43,0x1,0x6,0x6,0xd,0x43,0xa,0xe,0x13,0xf,0x6,0xe,0x6,0xd,0x17,0x6,0x7]

private func extensionAnd(make num: UInt8) -> UInt8 {
    return num ^ 99
}

/*: "Cancel" :*/
fileprivate let str_withTitle:String = "info data room toCancel"

/*: "Sure" :*/
fileprivate let str_viewValue:String = "Suresize else height private"
...
class PostView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_quoteMoreName.map{extensionAnd(make: $0)}, encoding: .utf8)!)
    }
    ...
    private func atWith() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(sumernationalise))
        ...
        cancelButton.addTarget(self, action: #selector(coatButton(button:)), for: .touchUpInside)
        ...
        confirmButton.addTarget(self, action: #selector(opticalLens(button:)), for: .touchUpInside)
    ...
    @objc func coatButton(button _: UIButton) {
        //: dismissView()
        sumernationalise()
    }

    //: @objc func confirmClick(button: UIButton) {
    @objc func opticalLens(button _: UIButton) {
        //: dismissView()
        sumernationalise()
    }
