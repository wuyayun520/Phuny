...
@objcMembers public class STLicHelper: NSObject {
    ...
    func checkLicenseFromLocal() -> Bool {
        if let path = KeyPodspecBundle.bundle.path(forResource: "SENSEME", ofType: "lic") {
...
extension STLicHelper {
    ...
    public func checkRemoteLicInfoWith(completion: @escaping Completionblock) {
        let reqModel = SafetyMarginReactiveCompatible.init()
    ...
    func checkLicFileMd5With(md5: NSString) -> Bool {
        ...
        let fileMd5: NSString = FailReactiveCompatible.shared.trademark(filePath: filePath as String) as NSString
