...
fileprivate let str_soundText:[Character] = ["\u{8bbe}","置","\u{666e}","\u{901a}","状","\u{6001}","的","文","\u{5b57}","颜","色","时"," ","请","使","用","R","G"]
fileprivate let str_managerTitle:String = "B\u{7a7a}间的颜色值"

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let str_leadingBindName:[Character] = ["设","置","选","中","\u{72b6}","\u{6001}","的","\u{6587}","字","\u{989c}","色"]
fileprivate let str_searchText:String = "时 请\u{4f7f}用RG"
fileprivate let str_frameTitle:[Character] = ["B","\u{7a7a}","间","的","颜","色","值"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let str_circleTitle:[UInt8] = [0x7b,0x7c,0x7b,0x66,0x3a,0x71,0x7d,0x76,0x77,0x60,0x28,0x3b,0x32,0x7a,0x73,0x61,0x32,0x7c,0x7d,0x66,0x32,0x70,0x77,0x77,0x7c,0x32,0x7b,0x7f,0x62,0x7e,0x77,0x7f,0x77,0x7c,0x66,0x77,0x76]

private func viewVarFile(label num: UInt8) -> UInt8 {
    return num ^ 18
}

/*: "设置的下标不合法!!" :*/
fileprivate let str_toValue:[Character] = ["设","置","的","下","标"]
fileprivate let str_positionText:String = "不合\u{6cd5}!!"

/*: "Bags" :*/
fileprivate let str_sizeName:String = "target titleBags"
...
open class ColorReactiveCompatible: UIView {
    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: RowMonthSegmentStyle
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.showRgb(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(str_soundText) + str_managerTitle))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.showRgb(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(str_leadingBindName) + str_searchText + String(str_frameTitle)))
        }
    ...
    fileprivate func showRgb(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: RowMonthSegmentStyle, titles: [String]) {
    ...
    @objc func clickOn(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? ErrorLabel else { return }
    ...
    public required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_circleTitle.map{viewVarFile(label: $0)}, encoding: .utf8)!)
    }
}
...
public extension ColorReactiveCompatible {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func full(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (String(str_toValue) + str_positionText))
...
extension ColorReactiveCompatible {
    ...
    fileprivate func currentFor() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = ErrorLabel(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.clickOn(_:)))
    ...
    fileprivate func state() {
        ...
        if let firstLabel = labelsArray[0] as? ErrorLabel {
    ...
    public func exhibit(_ currentIndex: Int) {
