...
- (void)setupViews {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(menuWillHide:)
                                               ...
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onNewMessage:) name:[AbradantNotificationTool colorNoti] object:nil];
...
- (void)loadMessage {
    ...
    if (self.currentChatType == TalkingIMChatType_group ||
        ...
        [[V2TIMManager sharedInstance] getGroupHistoryMessageList:self.targetID count:msgCount lastMsg:self.msgForGet succ:^(NSArray<V2TIMMessage *> *msgs) {
            ...
            if ([HiddenConversationListener shared].isConnection) { //推送点进来会报错
                NSString *str = [@"Net Error, Try again later" localized];
                [self noblesseExtraWithShowMsg:[str localized]];
            }
        ...
        [[V2TIMManager sharedInstance] getC2CHistoryMessageList:self.targetID count:msgCount lastMsg:self.msgForGet succ:^(NSArray<V2TIMMessage *> *msgs) {
            ...
            if ([HiddenConversationListener shared].isConnection) {                
                if (IndexStyleAppManager.share.appStatus == 0) {
                    NSString *str = [@"Net Error, Try again later" localized];
                    [self noblesseExtraWithShowMsg:[str localized]];
                }
            }
...
- (void)getMessages:(NSArray *)msgs msgCount:(int)msgCount {
    ...
    for (NSInteger k = array.count - 1; k >= 0; --k) {
        ...
        if ([userId isEqualToString:IndexStyleAppManager.share.loginUserMode.userID]) {
...
- (void)onNewMessage:(NSNotification *)notification
{
    ...
    if ([msg.sender isEqualToString:IndexStyleAppManager.share.loginUserMode.userID]) {
        //过滤本人发送的消息
        return;
    }
    ...
    if (uiMsgs.count > 0) {
        ...
        if ((self.tableView.contentSize.height - self.tableView.contentOffset.y) < [OverdoReactiveCompatible notReceive] * 1.5) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //    缓存存在问题待处理
    //    if (height > 0 && ![data isKindOfClass:[AbViewCellData class]] && ![data isKindOfClass:[AddCellData class]]) {
    //        return height;
    //    }
...
- (void)sendCustomMessage:(TUIMessageCellData *)msg {
    ...
    NSInteger sender = IndexStyleAppManager.share.loginUserMode.userID.integerValue;
    NSString *msgID;
    if (isGroup) {
...
- (void)modifyCustomMessageDataWithExtral:(NSDictionary *)extral cellData:(TUIMessageCellData *)msg {
    ...
    // 更新内存模型
    msg.msgModel = [LockModel modelWithDic:newExtraDict];
}
...
///   - msg: 图片消息
///   - model: 私密照片模型
- (void)changeIntimatePhotoMsg:(TUIMessageCellData *)msg photoModel:(OverrideModel *)model {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (model == nil) { return; }
        [OverrideModel to:model enableSend:msg.msgID];
...
///   - msg: 视频消息
///   - model: 私密视频模型
- (void)changeIntimateVideoMsg:(TUIMessageCellData *)msg videoModel:(ChiaroscuroModel *)model {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (model == nil) { return; }
        [ChiaroscuroModel need:model identity:msg.msgID];
...
- (void)changeIntimateGiftMsg:(TUIMessageCellData *)msg {
    dispatch_async(dispatch_get_main_queue(), ^{
        [AcceptJsonModel giftOf:msg.msgID];
...
- (TalkingSystemMsgCellData *)transSystemMsgFromDate:(NSDate *)date
{
    if(_msgForDate == nil || fabs([date timeIntervalSinceDate:_msgForDate.timestamp]) > MAX_MESSAGE_SEP_DLAY) {
        ...
        system.contentStr = [NSDate lengthyWithShowDetail:YES date:date];
...
- (void)onLongPressMessageWithCell:(TUIMessageCell *)cell{
    ...
    BOOL hasJumps = NO;
    if ([data isKindOfClass:[BackgroundLabelCellData class]]) {
        BackgroundLabelCellData *textCellData = (BackgroundLabelCellData *)data;
    ...
    // 只有文字才有复制功能
    if (([data isKindOfClass:[PresentationCellData class]] ||
         [data isKindOfClass:[BackgroundLabelCellData class]]) && hasJumps == NO) {
        // 过滤掉音视频通话文本消息
...
- (NSArray *)requiredResponseWithLongGestureRecognizer {
    return @[[PresentationCellData class],
             [BackgroundLabelCellData class],
             [ClearCellData class],
             [TalkingManagerCollectionCellData class],
             [TalkingCellData class],
             [CellData class],
             [GrantCellData class],
             [HiddenCellData class]];
}
...
- (void)onCopyMsg:(id)sender {
    ...
    if ([_menuUIMsg isKindOfClass:[PresentationCellData class]] ||
        [_menuUIMsg isKindOfClass:[BackgroundLabelCellData class]]) {
        BackgroundLabelCellData *txtMsg = (BackgroundLabelCellData *)_menuUIMsg;
...
- (BOOL)canReplyResponseWithLongGestureRecognizer:(TUIMessageCellData *)cellData {
    ...
    if ([cellData isKindOfClass:[CellData class]] || [cellData isKindOfClass:[ClearCellData class]] || [cellData isKindOfClass:[HiddenCellData class]]) {
        return YES;
    }
...
- (void)translateMsgWithCellData:(TUIMessageCellData *)cellData {
    ...
    if (IndexStyleAppManager.share.appUserConfigMode.enableTranslate == NO) { // 非会员，无法使用翻译
        [[PostPushManager share] nearQuitShow];
        return;
    }
    
    if ([cellData isKindOfClass:[BackgroundLabelCellData class]]) {
        BackgroundLabelCellData *txtMsg = (BackgroundLabelCellData *)cellData;
        [self translateMessageToReq:txtMsg andType:0];
        
    } else if ([cellData isKindOfClass:[PresentationCellData class]]) {
        PresentationCellData *txtMsg = (PresentationCellData *)cellData;
...
- (void)translateMessageToReq:(TUIMessageCellData *)txtMsg andType:(int)type {
    ...
    // 调用翻译接口
    [ChatMakeReactiveCompatible equalForMsgId:txtMsg.msgModel.msgInfo.msgId type:type completion:^(BOOL succeed, NSDictionary *dict, NSInteger errorCode) {
        if (!succeed) {
            if (errorCode == 403) { // 非会员，无法使用翻译
                IndexStyleAppManager.share.appUserConfigMode.enableTranslate = NO;
                [[PostPushManager share] nearQuitShow];
            }
