...
fileprivate let str_modeValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_chat_miss" :*/
fileprivate let str_prizeName:String = "sub fromicon_c"
fileprivate let str_barResponseValue:String = "toiss"

/*: "icon_chat_yes" :*/
fileprivate let str_viewRoomName:String = "cover height index cellicon_ch"
fileprivate let str_modelData:[Character] = ["a","t","_","y","e","s"]
...
class CommenceMsgCell: ChatView {
//    var transBlock: (() -> Void)?

    //: var textData: BackgroundLabelCellData?
    var textData: BackgroundLabelCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_modeValue.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var quoteView: DemonstrateReactiveCompatible = {
        //: let quoteView = TalkingMsgQuoteView()
        let quoteView = DemonstrateReactiveCompatible()
        //: quoteView.isHidden = false
        quoteView.isHidden = false
        //: quoteView.addTarget(self, action: #selector(quoteBtnClick), for: .touchUpInside)
        quoteView.addTarget(self, action: #selector(observerVideo), for: .touchUpInside)
        //: return quoteView
        return quoteView
        //: }()
    }()
    ...
    lazy var transLB: YYLabel = {
        ...
        //: label.numberOfLines = 0
        label.numberOfLines = 0
        //: label.font = UIFont.exceptEqual(fontSize: 16)
        label.font = UIFont.exceptEqual(fontSize: 16)
        //: return label
        return label
        //: }()
...
//        btn.setImage(UIImage.nearTap(name: "icon_chat_translate_nor"), for: .normal)
//        btn.setImage(UIImage.nearTap(name: "icon_chat_translate_pre"), for: .highlighted)
...
extension CommenceMsgCell {
    //: override func fill(with data: TCommonCellData) {
    override func fill(with data: TCommonCellData) {
        //: super.fill(with: data)
        super.fill(with: data)

        //: if let temTextData = data as? BackgroundLabelCellData {
        if let temTextData = data as? BackgroundLabelCellData {
            ...
            if imgPreview.count > 0, temTextData.msgModel.msgInfo.messageType == .MessageTypeWelcome {
                ...
                giftImagV.underPause(urlStr: imgPreview)
            }
            //: if temTextData.hasJump {
            if temTextData.hasJump {
                //: removeAllTapGes()
                magnitudeeractionCreate()
                //: addLongTagGes()
                stud()
                //: } else {
            } else {
...
//                self.messageData?.translateStatus != .MsgTranslate_succeed && self.messageData?.msgModel.msgInfo.callCmd.count ?? 0 <= 0 && temTextData.msgModel.msgInfo.messageType != .MessageTypeWelcome && "\(temTextData.msgModel.user.uid)" != OverdoReactiveCompatible.getXiaoMiID()) { // 对方发的 && 开启了翻译功能 && 消息未翻译成功
...
//            }
            ...
            if temTextData.msgModel.quoteMsgInfo.key() {
            ...
            if self.textData?.translateStatus == .MsgTranslate_translating {
                ...
                self.transShowView.stopAnimating()
                //: self.transStatusImagV.image = UIImage.nearTap(name: "icon_chat_miss")
                self.transStatusImagV.image = UIImage.nearTap(name: (String(str_prizeName.suffix(6)) + "hat_" + str_barResponseValue.replacingOccurrences(of: "to", with: "m")))
                //: } else if self.textData?.translateStatus == .MsgTranslate_succeed {
            } else if self.textData?.translateStatus == .MsgTranslate_succeed {
                //: self.transShowView.stopAnimating()
                self.transShowView.stopAnimating()
                //: self.transStatusImagV.image = UIImage.nearTap(name: "icon_chat_yes")
                self.transStatusImagV.image = UIImage.nearTap(name: (String(str_viewRoomName.suffix(7)) + String(str_modelData)))
            }
    ...
    func someTitle() {
        ...
        let containerTap = UITapGestureRecognizer(target: self, action: #selector(withTap(tap:)))
        ...
        self.container.addGestureRecognizer(containerTap)
        //: let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == OverdoReactiveCompatible.getXiaoMiID())
        let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == OverdoReactiveCompatible.winImage())
        //: if isOfficial == false {
        if isOfficial == false {
            //: addLongTagGes()
            stud()
