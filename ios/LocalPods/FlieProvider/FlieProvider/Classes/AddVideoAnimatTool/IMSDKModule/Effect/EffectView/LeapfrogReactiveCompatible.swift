...
fileprivate let str_beforeImageForName:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "iosSvgaEffect" :*/
fileprivate let str_skipData:String = "title status beiosSv"
fileprivate let str_insideValue:String = "gaEflock frame container right"

/*: "iosEffect" :*/
fileprivate let str_dataManagerTitle:String = "iosEffeheight let"
fileprivate let str_normalValue:[Character] = ["c","t"]

/*: "entityId" :*/
fileprivate let str_closedTitle:[Character] = ["e","n","t","i","t","y"]
fileprivate let str_labData:[Character] = ["I","d"]
...
// __CLOSE_PRINT__
//
//  LeapfrogReactiveCompatible.swift
//  AbroadTalking
//
...
class LeapfrogReactiveCompatible: UIView {
    //: var currentEffectMsgModel: TalkingAnimatMsgModel?
    var currentEffectMsgModel: SequenceMeasurable?
    ...
    var animatedView: ErrorItemView?

    //: open weak var delegate: AnimatViewStepProtocol?
    open weak var delegate: TitleStepProtocol?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_beforeImageForName.reversed(), encoding: .utf8)!)
    }
...
extension LeapfrogReactiveCompatible {
    //: func playAnimation(infoDic: Dictionary<String, Any>) {
    func communication(infoDic: [String: Any]) {
        //: let dict = infoDic
        let dict = infoDic
        //: if dict.keys.isEmpty == false {
        if dict.keys.isEmpty == false {
            //: var iosEffect: String = dict["iosSvgaEffect"] as! String
            var iosEffect: String = dict[(String(str_skipData.suffix(5)) + String(str_insideValue.prefix(4)) + "fect")] as! String
            //: if iosEffect.isEmptyString {
            if iosEffect.isEmptyString {
                //: iosEffect = dict["iosEffect"] as! String
                iosEffect = dict[(String(str_dataManagerTitle.prefix(7)) + String(str_normalValue))] as! String
            }

            //: let entityId = dict["entityId"] as! Int
            let entityId = dict[(String(str_closedTitle) + String(str_labData))] as! Int
            ...
            if iosEffect.isEmptyString == false {
                ...
                let successs = searchModel(msgModel: effectMsgModel)
            }
        }
    }
    ...
    func searchModel(msgModel: Any?) -> Bool {
        ...
        if msgModel is SequenceMeasurable {
            //: effectMsgModel = msgModel as? TalkingAnimatMsgModel
            effectMsgModel = msgModel as? SequenceMeasurable
            //: } else if msgModel is TalkingGiftMsgAnimatModel {
        } else if msgModel is PeriodOfTimeMeasurable {
            //: let giftMsgModel = msgModel as! TalkingGiftMsgAnimatModel
            let giftMsgModel = msgModel as! PeriodOfTimeMeasurable
            ...
            if giftMsgModel.animationTimes > 1 {
                ...
                tmpGiftMsgModel.titlePoint(giftNum: 1) // 礼物个数设置为1，每次动效显示
                ...
                var repeatEfftect = SequenceMeasurable()
            ...
            effectMsgModel = SequenceMeasurable()
            ...
            if giftMsgModel.showType == HalfColumnConvertible.myStery.rawValue || giftMsgModel.showType == HalfColumnConvertible.king.rawValue {
        ...
        if self.currentEffectMsgModel != nil || paused {
            ...
            if effectMsgModel?.top ?? false && self.effectMsgModelArray.count > 0 {
                ...
                for i in 0 ..< self.effectMsgModelArray.count {
                    ...
                    let aModel: SequenceMeasurable = self.effectMsgModelArray[i] as! SequenceMeasurable
                    //: if aModel.top == false {
                    if aModel.top == false {
                        //: index = i
                        index = i
                ...
                if index == -1 {
                    ...
                    self.effectMsgModelArray.add(effectMsgModel ?? SequenceMeasurable())
                    //: } else {
                } else {
                    //: self.effectMsgModelArray.insert(effectMsgModel ?? TalkingAnimatMsgModel.init(), at: index)
                    self.effectMsgModelArray.insert(effectMsgModel ?? SequenceMeasurable(), at: index)
                }
            ...
            viewError(effectMsgModel: effectMsgModel!)
        }
...
extension LeapfrogReactiveCompatible {
    //: func pause() {
    func commissionImage() {
        //: self.paused = true
        self.paused = true
        //: self.animatedView?.pauseAnimation()
        self.animatedView?.hemAndHaw()
    }
    ...
    func bluntResume() {
        ...
        if self.currentEffectMsgModel == nil {
            ...
            if self.effectMsgModelArray.count > 0 {
                ...
                guard let removeModel = getDown() else { return }
                //: firstShow(effectMsgModel: removeModel)
                viewError(effectMsgModel: removeModel)
            }
            //: } else {
        } else {
            //: self.animatedView?.resumeAnimation()
            self.animatedView?.sessionImage()
        }
    ...
    func getDown() -> SequenceMeasurable? {
        //: if (self.effectMsgModelArray.count == 0) {
        if self.effectMsgModelArray.count == 0 {
            //: return TalkingAnimatMsgModel.init()
            return SequenceMeasurable()
        }
        ...
        if effectMsgModel is SequenceMeasurable {
            //: let lastEffectMsgModel = effectMsgModel as! TalkingAnimatMsgModel
            let lastEffectMsgModel = effectMsgModel as! SequenceMeasurable
        ...
        return effectMsgModel as? SequenceMeasurable
    }
}
...
extension LeapfrogReactiveCompatible {
    ...
    func viewError(effectMsgModel: SequenceMeasurable) {
        ...
        self.delegate?.lowerClassSession(effectView: self, obj: effectMsgModel)

        //: TalkingGiftAnimatTool.shared.loadEffectData(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
        FailReactiveCompatible.shared.titleExecute(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
            ...
            self.isTo(effectMsgModel: effectMsgModel, success: success, effectModel: giftModel ?? TextModelType())
            //: })
        })
    }
    ...
    func isTo(effectMsgModel: SequenceMeasurable, success: Bool, effectModel: TextModelType) {
        // 资源下载完成
        //: self.delegate?.animatViewWithDidEndLoadingResource(effectView: self, obj: effectMsgModel, success: success)
        self.delegate?.limit(effectView: self, obj: effectMsgModel, success: success)
        ...
        if success {
            ...
            if self.paused {
                ...
                if self.currentEffectMsgModel?.eid == effectMsgModel.eid, self.currentEffectMsgModel?.effectUrl == effectMsgModel.effectUrl {
                    ...
                    theorySize(effectMsgModel: effectMsgModel, model: effectModel)
                    //: return
                    return
                }
            }
    ...
    func substitute(effectMsgModel: SequenceMeasurable, effectModel: TextModelType) {
        //: if self.animatedView != nil {
        if self.animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.changeAnimating()
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.spickPosition()
        ...
        className = FailReactiveCompatible.shared.label(oldName: className ?? "")
        ...
        let realClass = AnimatedViewClass as! ErrorItemView.Type
...
extension LeapfrogReactiveCompatible: UnfluctuatingViewDelegate {
    ...
    func tagInstance(effectItemView: ErrorItemView, success: Bool) {
        ...
        if !success {
            ...
            showNaturalEventSuccess(success: success)
        }
        ...
        self.delegate?.drape(effectView: self, obj: effectItemView.effectMsgModel ?? SequenceMeasurable(), success: success)
    }
    ...
    func landmarkView(effectItemView: ErrorItemView) {
        ...
        if effectItemView == self.animatedView {
            ...
            if !self.needStopAll {
                ...
                showNaturalEventSuccess(success: true)
            }
        }
        ...
        self.delegate?.buttonBuild(effectView: self, obj: effectMsgModel!)
    }
}
...
extension LeapfrogReactiveCompatible {
    ...
    func nameGroup() {
        ...
        if animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.changeAnimating() // stop animation
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.spickPosition() // clean animation data
    ...
    func showNaturalEventSuccess(success: Bool) {
        //: cleanData()
        nameGroup()
