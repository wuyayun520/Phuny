...
fileprivate let str_commitData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: _ :*/
fileprivate let str_showName:[Character] = ["_"]

/*: "UITableViewCell" :*/
fileprivate let str_viewName:String = "UITablprice group for manager"
fileprivate let str_failValue:String = "wCellcount window lock height"
...
class ServerBassCell: UITableViewCell {
    //: var msgModel = TalkingVideoCallDanmuModel.init()
    var msgModel = TrademarkModelType()
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        self.msgLabel.snp.makeConstraints { make in
            ...
            make.top.equalTo(consLet_barText + 2)
...
//            make.trailing.lessThanOrEqualTo(-32)
        ...
        self.bgLb.snp.makeConstraints { make in
            ...
            make.top.equalTo(consLet_barText)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_commitData.reversed(), encoding: .utf8)!)
    }
...
extension ServerBassCell {
    ...
    class func totalimate(tableView: UITableView, msg: TrademarkModelType, indexPath _: IndexPath) -> ServerBassCell {
        //: var cell: TalkingVideoDanmuMsgBassCell?
        var cell: ServerBassCell?

        //: if msg.messageType == 1 {
        if msg.messageType == 1 {
            //: let identifier = "\(TalkingVideoDanmuMsgTextCell.className())_\(msg.messageType)"
            let identifier = "\(LookerViewCell.className())_\(msg.messageType)"
            //: tableView.register(TalkingVideoDanmuMsgTextCell.self, forCellReuseIdentifier: identifier)
            tableView.register(LookerViewCell.self, forCellReuseIdentifier: identifier)
            //: cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingVideoDanmuMsgTextCell
            cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? LookerViewCell
            //: if cell == nil {
            if cell == nil {
                //: cell = TalkingVideoDanmuMsgTextCell(style: .default, reuseIdentifier: identifier)
                cell = LookerViewCell(style: .default, reuseIdentifier: identifier)
            }
            //: }else {
        } else {
            //: let identifier = "UITableViewCell"
            let identifier = (String(str_viewName.prefix(6)) + "eVie" + String(str_failValue.prefix(5)))
            //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier)
            var cell = tableView.dequeueReusableCell(withIdentifier: identifier)
            //: if cell == nil {
            if cell == nil {
                //: cell = TalkingVideoDanmuMsgBassCell(style: .default, reuseIdentifier: identifier)
                cell = ServerBassCell(style: .default, reuseIdentifier: identifier)
            }
            //: return cell as! TalkingVideoDanmuMsgBassCell
            return cell as! ServerBassCell
        }
