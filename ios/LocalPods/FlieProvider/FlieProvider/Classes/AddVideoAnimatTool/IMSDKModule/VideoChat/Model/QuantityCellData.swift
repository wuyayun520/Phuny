...
/*: ：" :*/
fileprivate let str_finishTitle:String = "\u{ff1a}"

/*: "#BAFBFF" :*/
fileprivate let str_headLimitVisibleValue:String = "type make self#BAFBFF"
...
class QuantityCellData: NSObject {
    ...
    var model = TrademarkModelType()

    //: func caculateMsgHeight(model: TalkingVideoCallDanmuModel) -> TalkingVideoCallDanmuModel {
    func minimize(model: TrademarkModelType) -> TrademarkModelType {
        //: self.model = model
        self.model = model
        /// 普通文本消息
    ...
    func recover(model: TrademarkModelType) -> TrademarkModelType {
        //: self.model = model
        self.model = model
        /// 普通文本消息
        //: if self.model.messageType == 1 {
    ...
    func lyric() {
        ...
        let paraStyle = previous()
        //: paraStyle.lineSpacing = 3.0
        paraStyle.lineSpacing = 3.0 // 行间距
        // 名字
        //: let firstStr = "\(self.model.nickname)："
        let firstStr = "\(self.model.nickname)："
        //: let name = getAttributed(text: firstStr, font: UIFont.little(fontSize: 14), color: UIColor.init(hex: "#BAFBFF")!)
        let name = appColor(text: firstStr, font: UIFont.little(fontSize: 14), color: UIColor(hex: "#BAFBFF")!)
        // 内容
        //: let content = getAttributed(text: self.model.content, font: UIFont.little(fontSize: 14), color: UIColor.white)
        let content = appColor(text: self.model.content, font: UIFont.little(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        ...
        exampleTitle(attribText: self.model.msgAttribText)
    }
    ...
    func sign() {
        ...
        let name = appColor(text: firstStr, font: UIFont.little(fontSize: 14), color: UIColor(hex: "#BAFBFF")!)
        // 内容
        //: let content = getAttributed(text: self.model.transContent, font: UIFont.little(fontSize: 14), color: UIColor.white)
        let content = appColor(text: self.model.transContent, font: UIFont.little(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        //: name.yy_paragraphStyle = paraStyle
        name.yy_paragraphStyle = paraStyle
    ...
    func previous() -> NSMutableParagraphStyle {
    ...
    func appColor(text: String, font: UIFont, color: UIColor) -> NSMutableAttributedString {
...
//            attribute.yy_writingDirection = [3]
        ...
        attribute.yy_paragraphStyle = previous()
        //: return attribute
        return attribute
    }
    ...
    func exampleTitle(attribText: NSMutableAttributedString) {
        // 距离左边8  距离右边也为8
        //: let maxWidth = VideoMsgTableViewWidth - 36
        let maxWidth = consLet_saveName - 36
