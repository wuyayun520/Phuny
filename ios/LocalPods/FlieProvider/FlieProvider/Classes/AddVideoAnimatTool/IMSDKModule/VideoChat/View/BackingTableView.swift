...
fileprivate let str_partySendLetName:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
@objc protocol FuturismObjectProtocol: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func talkGetWeaving()

    //: @objc optional func endScroll()
    @objc optional func barChange()
}

//: class TalkingVideoDanmuMsgListTableView: UIView {
class BackingTableView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(consultation))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_partySendLetName.reversed(), encoding: .utf8)!)
    }
...
extension BackingTableView {
    ...
    func timerTab() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(viewEvent), userInfo: nil, repeats: true)
    }
...
extension BackingTableView {
    ...
    func callBy(msgModel: TrademarkModelType?) {
    ...
    func bilgeShow() {
        ...
        if AllHeight > consLet_recordName {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < consLet_recordName {
                ...
                self.msgTableView.height = consLet_recordName
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = consLet_recordName - AllHeight
        ...
        climax(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func climax(animated: Bool) {
...
extension BackingTableView: UITableViewDelegate, UITableViewDataSource {
    ...
    func theLike(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.barChange?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(consLet_barText)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = ServerBassCell.totalimate(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension BackingTableView {
    ...
    func behindBy() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: consLet_saveName, height: consLet_recordName)
    }
}
