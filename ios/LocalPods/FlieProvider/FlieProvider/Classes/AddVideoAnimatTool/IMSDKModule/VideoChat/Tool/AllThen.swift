...
/*: "call_bgm" :*/
fileprivate let str_appAddTitle:String = "videol"
fileprivate let str_makeText:[Character] = ["l","_","b","g","m"]

/*: "call_response_bgm" :*/
fileprivate let str_executeValue:String = "call_type type false"
fileprivate let str_partyText:String = "se_bgself label count"
fileprivate let str_noName:String = "content"
...
enum BackgroundColumnConvertible: Int {
    //: case Call
    case Call /// 音视频拨打音乐
    //: case Response
...
class AllThen: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = BackgroundColumnConvertible.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func modelType(type: BackgroundColumnConvertible) -> AllThen {
...
extension AllThen {
    ...
    private func withNotifyError(notify: Notification) {
    ...
    private func windowFairwayListCellListenerNascency(notify: Notification) {
    ...
    private func deleteView() {
        ...
        if self.type == .Response {
            ...
            fileName = (String(str_executeValue.prefix(5)) + "respon" + String(str_partyText.prefix(5)) + str_noName.replacingOccurrences(of: "content", with: "m"))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = ResourceEffectTool.default.localType(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension AllThen: TXVodPlayListener {
