...
/*: "videodSzOun_gb_talk" :*/
fileprivate let str_playTitle:String = "vistatus"
fileprivate let str_styleData:String = "Oun_array for case succeed"

/*: "#171717" :*/
fileprivate let str_enterName:String = "#171717"

/*: "icon_cameraoff" :*/
fileprivate let str_addValue:String = "icon_cthe main selected result"
fileprivate let str_titleName:[Character] = ["o","f","f"]

/*: "%@ is answering now..." :*/
fileprivate let str_toEqualTitle:String = "view show total string self%@ is"
fileprivate let str_writeName:[Character] = [" ","a","n","s","w","e","r","i","n"]
fileprivate let str_managerText:[Character] = ["g"," ","n","o","w",".",".","."]

/*: "%@ turned off the camera" :*/
fileprivate let str_tableCurrentViewName:[Character] = ["%","@"," ","t","u","r","n","e","d"," ","o","f","f"," ","t","h","e"," ","c","a","m","e","r","a"]
...
class RecountView: UIView {
    ...
    lazy var bgView: UIImageView = {
        //: let v = UIImageView()
        let v = UIImageView()
        //: v.image = UIImage.nearTap(name: "videodSzOun_gb_talk")
        v.image = UIImage.nearTap(name: (str_playTitle.replacingOccurrences(of: "status", with: "de") + "odSz" + String(str_styleData.prefix(4)) + "gb_talk"))
    ...
    private lazy var headPic: UIImageView = {
        ...
        icon.contentMode = .scaleAspectFill
        //: icon.image = UIImage.nearTap(name: "icon_cameraoff")
        icon.image = UIImage.nearTap(name: (String(str_addValue.prefix(6)) + "amera" + String(str_titleName)))
        //: v.addSubview(icon)
        v.addSubview(icon)
    ...
    private lazy var coverView: UIView = {
        ...
        layer.colors = [UIColor.clear.cgColor, UIColor.computeImage(24, 19, 55, 0.9).cgColor]
        //: layer.locations = [0, 1]
        layer.locations = [0, 1]
...
extension RecountView {
    /// 是否展示模糊效果
    /// - Parameter show: 是否展示
    //: func showCoverView(show: Bool) {
    func phosphoresce(show: Bool) {
    ...
    func nameUp(urlStr: String?, showName: String?, isOpenCamera: ColorHalfCameraStatus) {
        ...
        if isOpenCamera == .Open {
            ...
            if isOpenCamera == .Normal {
                ...
                nickNameLab.text = (String(str_toEqualTitle.suffix(5)) + String(str_writeName) + String(str_managerText)).localizedArguments(showName!)
                //: } else {
            } else {
                //: nickNameLab.text = "%@ turned off the camera".localizedArguments(showName!)
                nickNameLab.text = (String(str_tableCurrentViewName)).localizedArguments(showName!)
            }
        }
    }
}
