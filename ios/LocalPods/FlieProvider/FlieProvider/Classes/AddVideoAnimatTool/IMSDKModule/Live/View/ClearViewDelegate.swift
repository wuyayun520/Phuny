...
fileprivate let str_colorNetworkContent:[UInt8] = [0x8e,0x89,0x8e,0x93,0xcf,0x84,0x88,0x83,0x82,0x95,0xdd,0xce,0xc7,0x8f,0x86,0x94,0xc7,0x89,0x88,0x93,0xc7,0x85,0x82,0x82,0x89,0xc7,0x8e,0x8a,0x97,0x8b,0x82,0x8a,0x82,0x89,0x93,0x82,0x83]
...
@objc protocol StepObjectProtocol: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func oversight()

    //: @objc optional func endScroll()
    @objc optional func managingDirector()
}

//: class TalkingDanmuMsgListTableView: UIView {
class ClearViewDelegate: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_colorNetworkContent.map{$0^231}, encoding: .utf8)!)
    }
...
extension ClearViewDelegate {
    ...
    func limitContent() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(legacy), userInfo: nil, repeats: true)
    }
...
extension ClearViewDelegate {
    ...
    func meanSunButtonRecent(msgModel: StoreHandyJSON?) {
    ...
    func penalise() {
        ...
        if AllHeight > consLet_statusDetailData {
            //: if self.msgTableView.height < MsgTableViewHeight {
            if self.msgTableView.height < consLet_statusDetailData {
                ...
                self.msgTableView.height = consLet_statusDetailData
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = consLet_statusDetailData - AllHeight
        ...
        positionAnimated(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func positionAnimated(animated: Bool) {
    ...
    func stochastic(inPending: Bool) {
...
extension ClearViewDelegate: UITableViewDelegate, UITableViewDataSource {
    ...
    func thirdConstraint(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.managingDirector?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(consLet_viewValue)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = StoreVersionReactiveCompatible.speech(tableView: tableView, msg: msgModel, indexPath: indexPath)
