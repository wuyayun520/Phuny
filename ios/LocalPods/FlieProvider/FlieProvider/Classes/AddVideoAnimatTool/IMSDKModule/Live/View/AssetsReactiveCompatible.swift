...
/*: "99+" :*/
fileprivate let str_voiceShareContent:String = "tabletable+"

/*: "init(coder:) has not been implemented" :*/
fileprivate let str_skinValue:[UInt8] = [0xcd,0xca,0xcd,0xd0,0x8c,0xc7,0xcb,0xc0,0xc1,0xd6,0x9e,0x8d,0x84,0xcc,0xc5,0xd7,0x84,0xca,0xcb,0xd0,0x84,0xc6,0xc1,0xc1,0xca,0x84,0xcd,0xc9,0xd4,0xc8,0xc1,0xc9,0xc1,0xca,0xd0,0xc1,0xc0]

/*: "#2DF2FF" :*/
fileprivate let str_centerValue:String = "#2DF2Fcase position index content"
fileprivate let str_selectedAspectAppData:[Character] = ["F"]

/*: "Live" :*/
fileprivate let str_inputTitle:String = "Livevar user name"

/*: "img_home_shadow_icon" :*/
fileprivate let str_dataLabTitle:String = "user self managerimg_"
fileprivate let str_mirrorData:String = "collection data viewshadow_"
fileprivate let str_dataValue:[Character] = ["i","c","o","n"]

/*: "get json error" :*/
fileprivate let str_playerFillValue:String = "get jsonrow share return super color"
fileprivate let str_eventText:String = " errormatch make max"

/*: "#FF2348" :*/
fileprivate let str_deadlineName:[Character] = ["#","F","F","2","3","4","8"]

/*: "#35C759" :*/
fileprivate let str_styleName:[Character] = ["#"]
fileprivate let str_enableData:String = "number info current35C759"

/*: "mfBean" :*/
fileprivate let str_magnitudeTitle:[Character] = ["m","f","B","e","a","n"]

/*: "%.1f" :*/
fileprivate let str_everySayContent:[Character] = ["%",".","1","f"]

/*: "points\n≈ $" :*/
fileprivate let str_tableName:String = "extrain"
fileprivate let str_addTitle:String = "ts\n≈ $"

/*:   :*/
fileprivate let str_imageValue:String = "color"
...
// __CLOSE_PRINT__
//
//  AssetsReactiveCompatible.swift
//  AbroadTalking
//
...
public var consLet_messageValue: Int {
    //: return IndexStyleAppManager.share.appConfigMode.countdown1 + IndexStyleAppManager.share.appConfigMode.countdown2
    return IndexStyleAppManager.share.appConfigMode.countdown1 + IndexStyleAppManager.share.appConfigMode.countdown2
}

//: class TalkingLiveMiniView: TalkingBaseMiniView {
class AssetsReactiveCompatible: CountStepView {
    // 未检测出人脸时长（间隔1秒）
    ...
    var noFaceTime: Int = 0 {
        ...
        didSet {
            ...
            if noFaceTime >= IndexStyleAppManager.share.appConfigMode.countdown1 { // 展示倒计时
                ...
                svgaNumLab.text = "\(consLet_messageValue - noFaceTime)"
                //: } else {
            } else {
                //: refreshMiniView()
                belowListMake()
    ...
    override init(frame: CGRect) {
        ...
        //: if Double(IndexStyleAppManager.share.loginUserMode.mf_bean) > 0 {
        if Double(IndexStyleAppManager.share.loginUserMode.mf_bean) > 0 {
            //: setPointText(mfBean: 0.0)
            smallFail(mfBean: 0.0)
        }
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_skinValue.map{$0^164}, encoding: .utf8)!)
    }
    ...
    private lazy var shadowImg: UIImageView = {
        ...
        img.image = UIImage.nearTap(name: (String(str_dataLabTitle.suffix(4)) + "home_" + String(str_mirrorData.suffix(7)) + String(str_dataValue)))
        //: img.layer.cornerRadius = 8
        img.layer.cornerRadius = 8
        //: img.layer.masksToBounds = true
        img.layer.masksToBounds = true
    ...
    private lazy var svgaPlayer: SVGAPlayer = {
        ...
        let url = ResourceEffectTool.default.butterflyEffect(type: .Live_countdown_bg)
        //: do {
        do {
            ...
            printLog(message: (String(str_playerFillValue.prefix(8)) + String(str_eventText.prefix(6))))
        }
        //: return player
        return player
        //: }()
    ...
    private lazy var svgaNumLab: UILabel = {
        //: let lab = UILabel()
        let lab = UILabel()
        //: lab.font = UIFont.little(fontSize: 20)
        lab.font = UIFont.little(fontSize: 20)
...
extension AssetsReactiveCompatible {
    /// 构建视频小窗口
    //: static func buildLiveMiniView() -> TalkingLiveMiniView {
    static func checkTitle() -> AssetsReactiveCompatible {
        //: let view = TalkingLiveMiniView(frame: .zero)
        let view = AssetsReactiveCompatible(frame: .zero)
        //: let position = TalkingSocketManager.shared.liveMiniPosition
        let position = JewishCalendarMonthThen.shared.liveMiniPosition
    ...
    func belowListMake() {
        ...
        liveStatusImgV.isHidden = !DecisionMakerReactiveCompatible.barIn().isLive
...
extension AssetsReactiveCompatible {
    ...
    private func towardEffect() {
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(pointchange(notification:)),
                                               selector: #selector(pointchangeOf(notification:)),
                                               //: name: LIVE_USER_POINT_CHANGE,
                                               name: consLet_callValue,
                                               //: object: nil)
                                               object: nil)
    }

    //: @objc private func pointchange(notification: NSNotification) {
    @objc private func pointchangeOf(notification: NSNotification) {
        //: let userinfo = notification.userInfo as! [String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: guard let mfBean = userinfo["mfBean"] as? Double else { return }
        guard let mfBean = userinfo["mfBean"] as? Double else { return }
        //: setPointText(mfBean: mfBean)
        smallFail(mfBean: mfBean)
    }

    //: func setPointText(mfBean: Double) {
    func smallFail(mfBean: Double) {
        //: let nowBean = IndexStyleAppManager.share.loginUserMode.mf_bean + mfBean
        let nowBean = IndexStyleAppManager.share.loginUserMode.mf_bean + mfBean
        //: IndexStyleAppManager.share.loginUserMode.mf_bean = nowBean
        IndexStyleAppManager.share.loginUserMode.mf_bean = nowBean
