...
/*: "#ffffff" :*/
fileprivate let str_centerData:String = "#fffffffamily center mode user"

/*: "/dist/loungePlus/index.html" :*/
fileprivate let str_willData:[Character] = ["/","d","i","s","t","/","l","o","u","n","g","e","P","l","u","s","/","i","n","d","e","x",".","h","t","m"]
fileprivate let str_titleWinName:String = "row"

/*: "https://" :*/
fileprivate let str_managerTitle:String = "hdelaydelayp"

/*: "http://" :*/
fileprivate let str_indexValue:String = "http://height kit view"
...
class TalkingModelThen: UIView {
    ...
    let Broadcast_TextColor = "#ffffff"
    //: let Broadcast_ContentFont: UIFont = UIFont.little(fontSize: 12)
    let Broadcast_ContentFont: UIFont = .little(fontSize: 12)
    ...
    convenience init(model: MedietyScreenModel?) {
        //: self.init()
        self.init()
    ...
    private func sinceEqual() {
        ...
        if danumuMsgModel.bgPic.count > 0 {
            ...
            skulk(withUrl: danumuMsgModel.bgPic)
        }
        ...
        self.backgroundImageView.cling(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.Broadcast_HeightOfBackgroundImageView {
                    ...
                    newImage = image.swageBlockQuality(with: CGSize(width: targetWidth, height: self.Broadcast_HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: MedietyScreenModel? {
        //: didSet {
        didSet {
            //: configModel()
            sinceEqual()
    ...
    var totalTime: CGFloat {
        ...
        return (consLet_errorDeviceHeightData + widthValue) * Broadcast_SpendTime / consLet_errorDeviceHeightData
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * Broadcast_LineSpendTime / ScreenWidth
        return widthValue * Broadcast_LineSpendTime / consLet_errorDeviceHeightData
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * Broadcast_SpendTime / ScreenWidth
        return widthValue * Broadcast_SpendTime / consLet_errorDeviceHeightData
    }
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(somewhere(_:)))
        //: return t
        return t
        //: }()
    }()
...
extension TalkingModelThen {
    /// 弹幕点击跳转事件
    //: @objc func tapAction(_ tapGesture: UITapGestureRecognizer) {
    @objc func somewhere(_: UITapGestureRecognizer) {
        ...
        //: case "1": // 个人中心
        case "1": // 个人中心
            //: PostPushManager.share.func__pushToUserDetailVC(uid: self.danumuMsgModel?.jumpKey)
            PostPushManager.share.miniskirt(uid: self.danumuMsgModel?.jumpKey)

        //: case "2": // 直播间
        case "2": // 直播间
            //: if let uid = self.danumuMsgModel?.jumpKey {
            if let uid = self.danumuMsgModel?.jumpKey {
                //: PostPushManager.share.func_audiencePushToLiveRoomVC(uid: uid, enterType: .unknown)
                PostPushManager.share.anTarget(uid: uid, enterType: .unknown)
            }
        ...
        case "3": // H5页面
            ...
            if let url = self.danumuMsgModel?.jumpKey {
                ...
                if url.contains((String(str_willData) + str_titleWinName.replacingOccurrences(of: "row", with: "l"))) {
                    //: PostPushManager.share.func__pushToSubscribePageWebVC()
                    PostPushManager.share.nearQuitShow()
                    //: return
                    return
                }
                //: let httpsUrl = IndexStyleAppManager.share.appConfigMode.urlH5Domain+url
                let httpsUrl = IndexStyleAppManager.share.appConfigMode.urlH5Domain + url
                //: let httpUrl = httpsUrl.replacingOccurrences(of: "https://", with: "http://")
                let httpUrl = httpsUrl.replacingOccurrences(of: "https://", with: "http://")
                //: PostPushManager.share.func__pushToWebVC(urlStr: httpUrl)
                PostPushManager.share.gildTheLilyCount(urlStr: httpUrl)
            }
