...
/*: s" :*/
fileprivate let str_resistanceText:String = "ret"

/*: "init(coder:) has not been implemented" :*/
fileprivate let str_callTitle:[UInt8] = [0x28,0x2f,0x28,0x35,0x69,0x22,0x2e,0x25,0x24,0x33,0x7b,0x68,0x61,0x29,0x20,0x32,0x61,0x2f,0x2e,0x35,0x61,0x23,0x24,0x24,0x2f,0x61,0x28,0x2c,0x31,0x2d,0x24,0x2c,0x24,0x2f,0x35,0x24,0x25]

/*: "bth_live_follow_pre" :*/
fileprivate let str_netContent:[Character] = ["b","t","h","_","l","i"]
fileprivate let str_taskTitle:String = "make"
fileprivate let str_equivalentToData:String = "start lete_fol"

/*: "btn_live_mini_nor" :*/
fileprivate let str_bottomData:String = "btn_load from click"
fileprivate let str_toName:String = "message any content normal party_mini_n"
fileprivate let str_labelValue:[Character] = ["o","r"]

/*: "#2DF2FF" :*/
fileprivate let str_skinBackValue:[Character] = ["#","2","D","F","2"]
fileprivate let str_fillTabValue:[Character] = ["F","F"]

/*: "Live" :*/
fileprivate let str_selectedData:String = "Liveclass label view self at"

/*: "#FF506D" :*/
fileprivate let str_normalData:[Character] = ["#"]
fileprivate let str_arrayText:String = "FF506cover"

/*: "  :*/
fileprivate let str_toTitle:String = "seed"

/*: "removeAttentionUid" :*/
fileprivate let str_colorName:String = "user let makeremoveA"
fileprivate let str_userValue:[Character] = ["t","t","e","n","t","i"]
fileprivate let str_nameText:[Character] = ["o","n","U","i","d"]

/*: "attentionUid" :*/
fileprivate let str_closeData:String = "attemode"
fileprivate let str_appText:String = "info"

/*: "source" :*/
fileprivate let str_requestText:[Character] = ["s","o","u","r","c","e"]

/*: "num" :*/
fileprivate let str_componentName:[Character] = ["n","u","m"]

/*: "UID: :*/
fileprivate let str_valueArrayName:String = "position extra model makeUID:"

/*: "btn_live_closed_nor" :*/
fileprivate let str_originNameValue:String = "btn_ldelay return normal"
fileprivate let str_objectTitle:String = "osed_norcolor make if"
...
class DoingThen: UIView {
    //: weak var weakVC: TalkingBaseViewController?
    weak var weakVC: ServiceRecognizerDelegate?
    // 右侧按钮点击Block
    ...
    var noFaceTime: Int = 0 {
        //: didSet {
        didSet {
            //: if noFaceTime >= IndexStyleAppManager.share.appConfigMode.countdown1 {
            if noFaceTime >= IndexStyleAppManager.share.appConfigMode.countdown1 { // 展示倒计时
                //: self.liveStatusImgV.isHidden = true
                self.liveStatusImgV.isHidden = true
                //: self.countdownView.isHidden = false
                self.countdownView.isHidden = false
                //: self.countdownLab.text = "\(MAX_NOFACETIME-noFaceTime)s"
                self.countdownLab.text = "\(consLet_messageValue - noFaceTime)s"
                // svga
                //: self.svgaView.isHidden = false
                self.svgaView.isHidden = false
                //: self.svgaView.refreshView(countdown: "\(MAX_NOFACETIME-noFaceTime)")
                self.svgaView.writeVideo(countdown: "\(consLet_messageValue - noFaceTime)")
                //: } else {
            } else {
    ...
    private var liveModel = DomicileModelType()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_callTitle.map{$0^65}, encoding: .utf8)!)
    }
    ...
    private lazy var iconBgBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(buttonCon), for: .touchUpInside)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var nickNameLab: UILabel = {
        ...
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.font = UIFont.little(fontSize: 11)
        lab.font = UIFont.little(fontSize: 11)
        //: return lab
        return lab
        //: }()
    ...
    private lazy var followBtn: UIButton = {
        ...
        //: btn.isHidden = true
        btn.isHidden = true
        //: btn.setImage(UIImage.nearTap(name: "bth_live_follow_pre"), for: .normal)
        btn.setImage(UIImage.nearTap(name: (String(str_netContent) + str_taskTitle.replacingOccurrences(of: "make", with: "v") + String(str_equivalentToData.suffix(5)) + "low_pre")), for: .normal)
        //: btn.addTarget(self, action: #selector(followButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(snapObject), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var miniBtn: UIButton = {
        ...
        btn.setImage(UIImage.nearTap(name: (String(str_bottomData.prefix(4)) + "live" + String(str_toName.suffix(7)) + String(str_labelValue))), for: .normal)
        //: btn.addTarget(self, action: #selector(miniButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(trunk), for: .touchUpInside)
    ...
    private lazy var liveStatusImgV: LabelPositionReactiveCompatible = {
        ...
        img.setImage(image.image(), for: .normal)
    ...
    private lazy var countdownLab: UILabel = {
        //: let lab = UILabel()
        let lab = UILabel()
        //: lab.font = UIFont.little(fontSize: 12)
        lab.font = UIFont.little(fontSize: 12)
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.text = " \(IndexStyleAppManager.share.appConfigMode.countdown2)s"
        lab.text = " \(IndexStyleAppManager.share.appConfigMode.countdown2)s"
    ...
    lazy var svgaView: SightReactiveCompatible = {
        //: let v = TalkingLiveCountdownView()
        let v = SightReactiveCompatible()
        //: v.isHidden = true
        v.isHidden = true
        //: return v
        return v
    ...
    private lazy var onlineNumBtn: UIButton = {
        ...
        btn.fullMoon(color: UIColor.black.withAlphaComponent(0.25), forState: .normal)
        ...
        btn.setTitleColor(.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.little(fontSize: 12)
        btn.titleLabel?.font = UIFont.little(fontSize: 12)
        //: return btn
        return btn
        //: }()
    ...
    private lazy var onlineListView: ColoringReactiveCompatible = {
        //: let v = TalkingLiveRoomOnlineListView()
        let v = ColoringReactiveCompatible()
        //: v.touchIconBtnBlock = { [weak self] uid in
        v.touchIconBtnBlock = { [weak self] uid in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: self.showUserCard(uid: uid)
            self.foolishness(uid: uid)
        }
        //: return v
        return v
        //: }()
...
extension DoingThen {
    ...
    @objc func labCompletion(uid: Int, isAttention: Bool, completion: ((_ succeed: Bool) -> Void)? = nil) {
        ...
        if isAttention {
            ...
            dict[(String(str_colorName.suffix(7)) + String(str_userValue) + String(str_nameText))] = uid
            //: } else {
        } else {
            //: dict["attentionUid"] = uid
            dict[(str_closeData.replacingOccurrences(of: "mode", with: "n") + "tionU" + str_appText.replacingOccurrences(of: "info", with: "id"))] = uid
            //: dict["source"] = "1"
            dict["source"] = "1"
        }
        //: TalkingChatRequestTool.req_atationTool(isAttention: !isAttention, params: dict) { succeed, result, errorModel in
        ChatMakeReactiveCompatible.statusElement(isAttention: !isAttention, params: dict) { succeed, _, _ in
            //: completion?(succeed)
            completion?(succeed)
        }
    }
...
extension DoingThen {
    ...
    @objc private func bindClick() {
        ...
        self.onlineListView.groupingId(chatGroupId: liveModel.chatGroupId)
    }

    /// 获取
    //: @objc private func getOnlineNumData() {
    @objc private func sizeData() {
        //: TalkingAudienceManager().req_liveRoomOnlineNum(chatGroupId: liveModel.chatGroupId, completion:{ succeed, result, errorModel in
        ServerReactiveCompatible().hadith(chatGroupId: liveModel.chatGroupId, completion: { succeed, result, _ in
    ...
    private func sizeTimer() {
        ...
        onlineNunTimer = Timer.scheduledTimer(timeInterval: 10, target: self, selector: #selector(sizeData), userInfo: nil, repeats: true)
    }

    //: func stopTimer() {
    func pullUpShortBlockLab() {
...
extension DoingThen {
    ...
    func restore(_ liveModel: DomicileModelType) {
        ...
        icon.cling(urlStr: liveModel.streamerInfo.headPic, placeImg: UIImage.numericalQuantityTheme(sex: liveModel.streamerInfo.sex), options: optionsInfo)
        // 是否是主播
        ...
        if isAnchor {
            //: refreshLiveStatus()
            messageRequest()
            //: miniBtn.setImage(UIImage.nearTap(name: "btn_live_mini_nor"), for: .normal)
            miniBtn.setImage(UIImage.nearTap(name: (String(str_bottomData.prefix(4)) + "live" + String(str_toName.suffix(7)) + String(str_labelValue))), for: .normal)
            //: refreshFollow(isAttention: true)
            version(isAttention: true)
            //: } else {
        } else {
            //: miniBtn.setImage(UIImage.nearTap(name: "btn_live_closed_nor"), for: .normal)
            miniBtn.setImage(UIImage.nearTap(name: (String(str_originNameValue.prefix(5)) + "ive_cl" + String(str_objectTitle.prefix(8)))), for: .normal)
            //: refreshFollow(isAttention: self.liveModel.streamerInfo.isAttention)
            version(isAttention: self.liveModel.streamerInfo.isAttention)
        }
    }
    ...
    @objc func trunk() {
        ...
        if isAnchor { // 改为小窗口
            ...
            DecisionMakerReactiveCompatible.barIn().computerFileName(smallWindow: true)
            //: currentViewController()?.navigationController?.popViewController(animated: false)
            userRead()?.navigationController?.popViewController(animated: false)
        }
    }

    //: @objc private func iconBgButtonClick() {
    @objc private func buttonCon() {
        //: showUserCard(uid: String(TalkingLiveManager.shared().liveRoomModel.streamerInfo.uid))
        foolishness(uid: String(DecisionMakerReactiveCompatible.barIn().liveRoomModel.streamerInfo.uid))
    }

    /// 展示个人资料卡
    //: private func showUserCard(uid: String) {
    private func foolishness(uid: String) {
        //: let userCard = TalkingLiveUserCardView(frame: .zero, uid: uid)
        let userCard = ToCardView(frame: .zero, uid: uid)
        //: userCard.delegate = weakVC as? any TalkingLiveUserCardViewDelegate
        userCard.delegate = weakVC as? any TotalerpretationViewDelegate
        //: userCard.show()
        userCard.equalPlayerShow()
    }

    /// 刷新关注按钮
    //: private func refreshFollow(isAttention: Bool) {
    private func version(isAttention: Bool) {
    ...
    private func messageRequest() {
        //: self.liveStatusImgV.isHidden = !TalkingLiveManager.shared().isLive
        self.liveStatusImgV.isHidden = !DecisionMakerReactiveCompatible.barIn().isLive
    ...
    @objc private func snapObject() {
        //: req_followRequest(uid: liveModel.streamerInfo.uid, isAttention: liveModel.streamerInfo.isAttention) { [weak self] succeed in
        labCompletion(uid: liveModel.streamerInfo.uid, isAttention: liveModel.streamerInfo.isAttention) { [weak self] succeed in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: guard succeed else { return }
            guard succeed else { return }
            ...
            self.version(isAttention: self.liveModel.streamerInfo.isAttention)
        }
    }
}
...
extension DoingThen {
    ...
    private func accumulation() {
        ...
        self.frame = CGRect(x: 0, y: 0, width: consLet_errorDeviceHeightData, height: consLet_netViewValue + actualWidth(w: 68))
