...
fileprivate let str_tabData:[UInt8] = [0x5a,0x5d,0x5a,0x47,0x1b,0x50,0x5c,0x57,0x56,0x41,0x9,0x1a,0x13,0x5b,0x52,0x40,0x13,0x5d,0x5c,0x47,0x13,0x51,0x56,0x56,0x5d,0x13,0x5a,0x5e,0x43,0x5f,0x56,0x5e,0x56,0x5d,0x47,0x56,0x57]

private func playerFrame(label num: UInt8) -> UInt8 {
    return num ^ 51
}

/*: "no data available" :*/
fileprivate let str_whiteControlTitle:String = "no color equal"
fileprivate let str_liveData:String = "dindexa"
fileprivate let str_imageName:String = "asizele"

/*: "list" :*/
fileprivate let str_dailySharedAgentValue:[Character] = ["l","i","s","t"]
...
class DeviceViewDelegate: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_tabData.map{playerFrame(label: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(sizeDownCurrent), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var emptyView: UILabel = {
        ...
        lab.text = (String(str_whiteControlTitle.prefix(3)) + str_liveData.replacingOccurrences(of: "index", with: "at") + " avail" + str_imageName.replacingOccurrences(of: "size", with: "b")).localized
        //: lab.textColor = .white
        lab.textColor = .white
...
extension DeviceViewDelegate {
    ...
    @objc private func sizeDownCurrent() {
        //: dismiss()
        disappearDismiss()
    }
    ...
    private func crucifyHeight() -> CGFloat {
        ...
        return lineNum * consLet_giftValue + 24.0 + consLet_halfTitle
    }
}
...
extension DeviceViewDelegate: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.stopAdd(model: dataArr[indexPath.row])
        //: return cell
        return cell
    }
...
extension DeviceViewDelegate {
    ...
    func demonstrateLoad(from: ThriftshopFromType) {
        //: switch from {
        switch from {
        //: case .LiveRoom:
        case .LiveRoom:
            //: dataArr = TalkingLiveManager.shared().liveRoomModel.gameList
            dataArr = DecisionMakerReactiveCompatible.barIn().liveRoomModel.gameList
            //: showAnimation()
            priceAnimation()
    ...
    private func priceAnimation() {
        ...
        self.contentView.frame = CGRect(x: 0, y: consLet_sessionName, width: consLet_errorDeviceHeightData, height: crucifyHeight())
        //: UIView.animate(withDuration: 0.3) {
        UIView.animate(withDuration: 0.3) {
            //: self.contentView.frame.origin.y -= self.contentView.height
            self.contentView.frame.origin.y -= self.contentView.height
...
extension DeviceViewDelegate {
    ...
    private func publication() {
        ...
        contentView.frame = CGRect(x: 0, y: consLet_sessionName, width: consLet_errorDeviceHeightData, height: crucifyHeight())
