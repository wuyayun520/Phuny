...
fileprivate let str_listIndexText:[UInt8] = [0xa4,0xa9,0xa4,0xaf,0x63,0x9e,0xaa,0x9f,0xa0,0xad,0x75,0x64,0x5b,0xa3,0x9c,0xae,0x5b,0xa9,0xaa,0xaf,0x5b,0x9d,0xa0,0xa0,0xa9,0x5b,0xa4,0xa8,0xab,0xa7,0xa0,0xa8,0xa0,0xa9,0xaf,0xa0,0x9f]

fileprivate func dataUser(mobile num: UInt8) -> UInt8 {
...
/*: "bth_live_follow_pre" :*/
fileprivate let str_modelData:[Character] = ["b","t","h","_","l","i"]
fileprivate let str_errorValue:[Character] = ["v","e"]
fileprivate let str_labContent:String = "icon self cell_fol"

/*: "btn_live_closed_nor" :*/
fileprivate let str_kindContent:String = "btn_livwith image black"
fileprivate let str_pictureValue:[Character] = ["e","_","c","l","o","s"]
fileprivate let str_nameValue:String = "map vared_nor"

/*: "removeAttentionUid" :*/
fileprivate let str_indicatorData:[Character] = ["r","e","m","o","v","e","A","t","t","e","n","t","i","o","n","U"]
fileprivate let str_statusMenuData:String = "m"

/*: "attentionUid" :*/
fileprivate let str_sizeName:String = "ATTENT"
fileprivate let str_errImageData:String = "no"

/*: "source" :*/
fileprivate let str_aspectData:[Character] = ["s","o","u","r","c","e"]

/*: "num" :*/
fileprivate let str_actualValue:String = "nupush"

/*: "99+" :*/
fileprivate let str_storageEventTitle:[UInt8] = [0xaa,0xaa,0xb8]

private func cameraTo(maker num: UInt8) -> UInt8 {
    return num ^ 147
}

/*: "UID: :*/
fileprivate let str_languageRestoreValue:[Character] = ["U","I","D",":"]
...
class ObserverView: UIView {
    ...
    private var partyModel = KeyModelType()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_listIndexText.map{dataUser(mobile: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var iconBgBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(plusCount), for: .touchUpInside)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var followBtn: UIButton = {
        ...
        btn.setImage(UIImage.nearTap(name: (String(str_modelData) + String(str_errorValue) + String(str_labContent.suffix(4)) + "low_pre")), for: .normal)
        //: btn.addTarget(self, action: #selector(followButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(imageSection), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var miniBtn: UIButton = {
        //: let btn = UIButton()
        let btn = UIButton()
        //: btn.setImage(UIImage.nearTap(name: "btn_live_closed_nor"), for: .normal)
        btn.setImage(UIImage.nearTap(name: (String(str_kindContent.prefix(7)) + String(str_pictureValue) + String(str_nameValue.suffix(6)))), for: .normal)
        //: btn.addTarget(self, action: #selector(miniButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(textTooTop), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var onlineListView: EventViewDelegate = {
        //: let v = TalkingVoiceRoomOnlineListView()
        let v = EventViewDelegate()
...
extension ObserverView {
    ...
    @objc func shoveCompletion(uid: Int, isAttention: Bool, completion: ((_ succeed: Bool) -> Void)? = nil) {
        ...
        if isAttention {
            ...
            dict[(String(str_indicatorData) + str_statusMenuData.replacingOccurrences(of: "m", with: "id"))] = uid
            //: } else {
        } else {
            //: dict["attentionUid"] = uid
            dict[(str_sizeName.lowercased() + "ionU" + str_errImageData.replacingOccurrences(of: "no", with: "id"))] = uid
            //: dict["source"] = "2"
            dict["source"] = "2"
        }
...
extension ObserverView {
    ...
    @objc private func towardRangeVideo() {
        //: TalkingVoiceRoomReqTool.req_partyOnlineNum(roomId: partyModel.roomId) { succeed, result, _ in
        HeritageReqTool.top(roomId: partyModel.roomId) { succeed, result, _ in
    ...
    private func actionLoad() {
        ...
        onlineNunTimer = Timer.scheduledTimer(timeInterval: 60, target: self, selector: #selector(towardRangeVideo), userInfo: nil, repeats: true)
        //: onlineNunTimer?.fire()
        onlineNunTimer?.fire()
    }
...
extension ObserverView {
    ...
    func everyMiniView() {
        ...
        partyModel = EqualSendReactiveCompatible.clout().partyModel
        ...
        icon.cling(urlStr: partyModel.streamerInfo.headPic, placeImg: UIImage.numericalQuantityTheme(sex: partyModel.streamerInfo.sex), options: optionsInfo)
        // 是否是主播
        //: let isAnchor = (String(partyModel.streamerInfo.uid) == IndexStyleAppManager.share.loginUid)
        let isAnchor = (String(partyModel.streamerInfo.uid) == IndexStyleAppManager.share.loginUid)
        //: if isAnchor {
        if isAnchor {
            //: refreshFollow(isAttention: true)
            telephoneMessage(isAttention: true)
            //: } else {
        } else {
            //: refreshFollow(isAttention: partyModel.streamerInfo.isAttention)
            telephoneMessage(isAttention: partyModel.streamerInfo.isAttention)
        }
        // 定时更新在线人数
        //: startTimer()
        actionLoad()
    ...
    @objc private func imageSection() {
        //: req_followRequest(uid: partyModel.streamerInfo.uid, isAttention: partyModel.streamerInfo.isAttention) { [weak self] succeed in
        shoveCompletion(uid: partyModel.streamerInfo.uid, isAttention: partyModel.streamerInfo.isAttention) { [weak self] succeed in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: guard succeed else { return }
            guard succeed else { return }
            ...
            self.telephoneMessage(isAttention: self.partyModel.streamerInfo.isAttention)
        }
    }

    /// 最小化按钮点击事件
    //: @objc private func miniButtonClick() {
    @objc private func textTooTop() {
    ...
    func telephoneMessage(isAttention: Bool) {
    ...
    @objc private func sharedClick() {
        ...
        onlineListView.nooksAndCrannies()
    }
}
...
extension ObserverView {
    ...
    private func clickPrice() {
        //: frame = CGRect(x: 0, y: 0, width: ScreenWidth, height: StatusBarHeight+actualWidth(w: 43))
        frame = CGRect(x: 0, y: 0, width: consLet_errorDeviceHeightData, height: consLet_netViewValue + actualWidth(w: 43))
