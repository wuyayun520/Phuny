...
fileprivate let str_arrayValue:[UInt8] = [0xc3,0xc4,0xc3,0xde,0x82,0xc9,0xc5,0xce,0xcf,0xd8,0x90,0x83,0x8a,0xc2,0xcb,0xd9,0x8a,0xc4,0xc5,0xde,0x8a,0xc8,0xcf,0xcf,0xc4,0x8a,0xc3,0xc7,0xda,0xc6,0xcf,0xc7,0xcf,0xc4,0xde,0xcf,0xce]

/*: "SHBDropMenuCell" :*/
fileprivate let str_pairName:[Character] = ["S","H","B","D","r","o","p"]
fileprivate let str_tagValue:String = "Meopen type"
fileprivate let str_blockPopValue:String = "make self centernuCell"

/*: "title" :*/
fileprivate let str_sizeValue:String = "maketle"
...
public class SequenceDataSource: UIView {
    //: var menuDelegate: DropDownMenuViewDelegate?
    var menuDelegate: StoreViewDelegate?
    ...
    var menuBgColor: UIColor? {
        ...
        //: }
    } // = Config.AcousticPowerConfig.menuBgColor
    // 线条颜色
    //: var lineColor = Config.MenuCellConfig.lineColor
    var lineColor = Config.OrientationConfig.lineColor
    // cell高度
    //: public var menuCellHeight = Config.MenuCellConfig.menuCellHeight
    public var menuCellHeight = Config.OrientationConfig.menuCellHeight
    /**
     *  table最大高度限制
     *  默认：5 * cellHeight
     */
    //: var menuMaxHeight = Config.MenuCellConfig.menuMaxHeight
    var menuMaxHeight = Config.OrientationConfig.menuMaxHeight
    /**
     *  小三角高度
     *  45°等腰三角形
     */
    //: public var triangleHeight = Config.DropMenuConfig.triangleHeight
    public var triangleHeight = Config.AcousticPowerConfig.triangleHeight
    /**
     *  调整使下拉优先 当向下偏转屏幕距离足够，优先向下偏转
     */
    //: public var adjustPullDown = Config.DropMenuConfig.adjustPullDown
    public var adjustPullDown = Config.AcousticPowerConfig.adjustPullDown
    /**
     *  pullMenu样式
     */
    //: public var menuStyle = Config.initialMenuStyle ?? DropMenuStyle.MenuDarkStyle {
    public var menuStyle = Config.initialMenuStyle ?? IndexAtMenuStyle.MenuDarkStyle {
        //: didSet {
        didSet {
            //: switch menuStyle {
            switch menuStyle {
...
//        menuView.refreshUI()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_arrayValue.map{$0^170}, encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func releaseItem() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func exaggerateBackground() -> UITableView {
    ...
    private func noTable() {
        ...
        mTable.register(RoomWhiteReactiveCompatible.self, forCellReuseIdentifier: (String(str_pairName) + String(str_tagValue.prefix(2)) + String(str_blockPopValue.suffix(6))))
        //: contentView.addSubview(mTable)
        contentView.addSubview(mTable)
    }

    //: private func refreshUI() {
    private func actionUi() {
        //: drawmTableFrame()
        calendarDown()
        //: mTable.reloadData()
        mTable.reloadData()
    ...
    private func cacuateList() -> CGFloat {
...
extension SequenceDataSource: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(str_pairName) + String(str_tagValue.prefix(2)) + String(str_blockPopValue.suffix(6))), for: indexPath) as! RoomWhiteReactiveCompatible
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        menuDelegate?.star(index: indexPath.row, title: cellModel.title ?? "")
    }
}
