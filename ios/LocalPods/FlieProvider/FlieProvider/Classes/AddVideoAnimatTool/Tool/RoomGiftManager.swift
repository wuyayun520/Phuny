...
/*: "gifts" :*/
fileprivate let str_shareData:String = "errorifts"

/*: "vipGifts" :*/
fileprivate let str_ofName:String = "type height bottom let floatvipGift"
fileprivate let str_tableData:[Character] = ["s"]

/*: "packGifts" :*/
fileprivate let str_errorData:[Character] = ["p","a","c","k","G","i","f","t","s"]

/*: "giftSendItem" :*/
fileprivate let str_nameValue:[Character] = ["g","i","f","t","S","e","n","d","I","t","e","m"]
...
class RoomGiftManager: NSObject {
    /// 是否展示背包红点
    ...
    private var cacheDict = [String: [OverdoPriorityTarget: [Any]]]()
    // 数据锁
    ...
    private var giftType = JumpBasicType.normal

    //: static let share = TalkingChatGiftManager()
    static let share = RoomGiftManager()
...
extension RoomGiftManager {
    ...
    func indexMake(type: JumpBasicType = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        RoomGiftManager.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[OverdoPriorityTarget.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            RoomGiftManager.share.eventView(completion: { isSucceed in
            ...
            RoomGiftManager.share.moveGesture(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            RoomGiftManager.share.effectCompletion(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            RoomGiftManager.share.inputTime(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func inputTime(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = RoomGiftManager.share.giftType
        //: TalkingChatRequestTool.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        ChatMakeReactiveCompatible.run(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [OverdoPriorityTarget: [Any]]()
                // hot礼物
                //: if let array: Array = data["gifts"] as? Array<Any> {
                if let array: Array = data["gifts"] as? [Any] {
                    //: var hotArr = [Any]()
                    var hotArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<AwardModelType>.deserialize(from: array as? Array) {
                        //: hotArr.append(contentsOf: datas as [Any])
                        hotArr.append(contentsOf: datas as [Any])
                    }
                    //: dataDict[GiftDataType.Hot] = hotArr
                    dataDict[OverdoPriorityTarget.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(String(str_ofName.suffix(7)) + String(str_tableData))] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<AwardModelType>.deserialize(from: array as? Array) {
                    ...
                    dataDict[OverdoPriorityTarget.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(String(str_errorData))] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<AwardModelType>.deserialize(from: array as? Array) {
                    ...
                    dataDict[OverdoPriorityTarget.Bags] = bagsArr
                }
                ...
                var firstModel = FreebeeMeasurable()
                //: firstModel.num = 1
                firstModel.num = 1
                ...
                let sendItem: Array = data[(String(str_nameValue))] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<FreebeeMeasurable>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func eventView(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = RoomGiftManager.share.giftType
        //: TalkingChatRequestTool.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        ChatMakeReactiveCompatible.codex(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func lineToText(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: TalkingChatRequestTool.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        ChatMakeReactiveCompatible.numberernaliseList(params: params, completion: { succeed, result, _ in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<TotalensityMemberModel>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func moveGesture(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = JumpBasicType.live
        //: TalkingChatRequestTool.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        ChatMakeReactiveCompatible.createPackage(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    private func effectCompletion(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.party
        let type = JumpBasicType.party
        //: TalkingChatRequestTool.req_partyGiftList(type: type.rawValue) { succeed, result, errorModel in
        ChatMakeReactiveCompatible.oversightCompletion(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func needAcross(perPageCount: Int, dataType: OverdoPriorityTarget) -> Int {
    ...
    func giftViewType(dataType: OverdoPriorityTarget) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func momentDown() -> [Any] {
        ...
        if RoomGiftManager.share.giftType == .groupChat {
            //: return chatRoomSendItems
            return chatRoomSendItems
            //: } else if TalkingChatGiftManager.share.giftType == .live || TalkingChatGiftManager.share.giftType == .party {
        } else if RoomGiftManager.share.giftType == .live || RoomGiftManager.share.giftType == .party {
    ...
    func startPhoto(dataType: OverdoPriorityTarget, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension RoomGiftManager {
    ...
    @objc private func sheaf() {
        //: gift_clearMemoryCache(showRed: true)
        freebeeWithCherryRed(showRed: true)
    }
}
...
extension RoomGiftManager {
    ...
    func freebeeWithCherryRed(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        RoomGiftManager.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
